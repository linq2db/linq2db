...
- `/audit-agents` -- static audit of the `.claude/` corpus: seven check categories (dead reference, slnx mismatch, template gap, duplicated rule, retired path, terminology drift, refactor candidate); per-finding mechanical/creative/manual-only patches. Check definitions factored into `audit-agents-checks.md`.
...
| `audit-agents-checks.md` | Per-check rules for `/audit-agents` step 2: dead-reference, slnx-mismatch, template-gap, duplicated-rule, retired-path, terminology-drift, refactor-candidate |
...
- `diff-reader.ps1` -- batch file content + unified diff + hunk reader; `writeDir` writes HEAD/base/diff bodies to `.build/.agents/pr<n>/`; `include.styleScan` returns trailing-whitespace / 3+-blank-line / mixed-indent findings scoped to PR hunks.
...
- `test-status.ps1` -- reads the `LINQ2DB_TEST_PROGRESS` heartbeat JSON (`.build/.agents/test-progress.<tfm>.<pid>.json`); prints a one-line progress summary (`[RUNNING/DONE <tfm>] completed/total (%) | pass/fail/skip | rate | elapsed | eta | currentTest`) or raw JSON with `-Raw`. Used by `/test-progress` status action and recommended during long runs.
