...
- `LinqExtensions` -- the `[PublicAPI]` static partial class in namespace `LinqToDB`, split across thirteen files. Every public extension method is here. The class is the canonical entry point for non-IQueryable LINQ-style operations.
...
- `IEntityInsertSpec<T>` / `IEntityUpdateSpec<T>` / `IUpsertSpec<T>` -- expression-tree-only fluent configuration interfaces for the entity-shaped `Insert`/`Update`/`Upsert` overloads (PR #5482, issue #2558). Declared in `LinqToDB.Linq` (`Source/LinqToDB/Linq/IEntityInsertSpec.cs`, `IEntityUpdateSpec.cs`, `IUpsertSpec.cs`), not `LinqExtensions/`. The configure lambda is parsed, never invoked; implementing these interfaces is not a supported extension point. `IEntityInsertSpec<TTarget,TBuilder>` / `IEntityUpdateSpec<TTarget,TBuilder>` are F-bounded bases shared with the Upsert INSERT/UPDATE branches (`IUpsertInsertSpec<T>` / `IUpsertUpdateSpec<T>`).
...
All thirteen files are partials of one class, `LinqToDB.LinqExtensions`. The split is by feature area, not by visibility. Every extension method follows the same recipe:
...
- `LinqExtensions.EntityInsert.cs` -- entity-shaped `Insert<T>(ITable<T>, T item, Expression<Func<IEntityInsertSpec<T>,IEntityInsertSpec<T>>> configure)` / `InsertAsync`. Whole-object defaults are written from `item`; `.Set`/`.Ignore` overlay on top. Consumed by `EntityInsertBuilder` (`Source/LinqToDB/Internal/Linq/Builder/EntityInsertBuilder.cs`), which synthesises the existing `Insert<T>(ITable<T>, Expression<Func<T>>)` shape and defers to `InsertBuilder`. The async overload embeds the *sync* `Insert` `MethodInfo` in the captured expression tree (not `InsertAsync`) -- async-ness comes from `ExecuteAsync`. PR #5482 (issue #2558).
- `LinqExtensions.EntityUpdate.cs` -- entity-shaped `Update<T>(ITable<T>, T item, Expression<Func<IEntityUpdateSpec<T>,IEntityUpdateSpec<T>>> configure)` / `UpdateAsync`. Match is by primary key; whole-object defaults (every non-PK, non-skip-on-update column) come from `item`, overlaid by `.Set`/`.Ignore`. Consumed by `EntityUpdateBuilder` (`Source/LinqToDB/Internal/Linq/Builder/EntityUpdateBuilder.cs`), which synthesises the existing `q.Update(predicate, setter)` shape. Same sync-`MethodInfo`-embedding pattern as `EntityInsert`. PR #5482.
- `LinqExtensions.Upsert.cs` -- fluent `Upsert<T>`/`UpsertAsync<T>` family: bare `Upsert(target, item)` (all columns, PK match, via cached `UpsertIdentity<T>.Instance` identity lambda), the `IUpsertSpec<T>`-configured overload (`.Match`/`.Set`/`.Ignore`/`.SkipInsert`/`.SkipUpdate`/`.Insert(...)`/`.Update(...)`), plus `IEnumerable<T>` bulk-source and `IQueryable<T>` server-side-source overloads. `UpsertBuilder` (`Source/LinqToDB/Internal/Linq/Builder/UpsertBuilder.cs`) emits a native single-statement upsert (`ON CONFLICT`), a synthesised `Merge` call chain, or an emulated multi-statement `SELECT`->`UPDATE`->`INSERT` fallback depending on provider capability and configuration; `LinqOptions.UpsertEmulationPolicy` can reject the emulated fallback. PR #5482 (issue #2558).
- `LinqExtensions.LoadWith.cs` -- `LoadWithAsTable`, `LoadWith<TEntity,TProperty>` (4 overloads), `ThenLoad<TEntity,TPrev,TProperty>` (4 overloads). The `LoadWithQueryableBase<TEntity>` and `LoadWithQueryable<TEntity,TProperty>` private classes wrap an `IExpressionQuery<TEntity>`. Also hosts the eager-loading strategy marker methods `WithUnionLoadStrategy<T>`/`WithSeparateLoadStrategy<T>`/`WithKeyedLoadStrategy<T>` (translation-time markers only -- no runtime effect on the source query): CTE+UNION ALL, per-association separate query (`EagerLoadingStrategy.Default`), and buffered-parent-keys batch query respectively (PR #5450). An explicit per-query marker beats `LinqOptions.DefaultEagerLoadingStrategy`; when more than one marker is applied, the outermost-applied one wins. `LoadWithQueryableBase<TEntity>.NotLinqToDbSource` backs a documented passthrough: when the underlying query is not a linq2db query, `LoadWith`/`ThenLoad` return the source unchanged instead of throwing, mirroring EF Core `Include` (fix for an `InvalidCastException`, PR #5658).
...
| 1 | `Source/LinqToDB/LinqExtensions/LinqExtensions.cs` | Root partial: scalar-select, paging, nulls-position ordering, joins, CTE, set ops, DisableGuard, AggregateExecuteAsync, named IgnoreFilters |
...
| 2 | `Source/LinqToDB/LinqExtensions/LinqExtensions.LoadWith.cs` | `LoadWith`/`ThenLoad`/`LoadWithAsTable`/eager-loading strategy markers partial |
...
| 2 | `Source/LinqToDB/LinqExtensions/LinqExtensions.EntityInsert.cs` | Entity-shaped `Insert`/`InsertAsync` via `IEntityInsertSpec<T>` fluent configure (PR #5482) |
| 2 | `Source/LinqToDB/LinqExtensions/LinqExtensions.EntityUpdate.cs` | Entity-shaped `Update`/`UpdateAsync` via `IEntityUpdateSpec<T>`, PK-matched (PR #5482) |
| 2 | `Source/LinqToDB/LinqExtensions/LinqExtensions.Upsert.cs` | Fluent `Upsert`/`UpsertAsync` family (item / IEnumerable / IQueryable sources) via `IUpsertSpec<T>` (PR #5482) |
...
- `LinqToDB.Internal.Linq.Builder.*` (`Methods.LinqToDB.*` -- pre-resolved `MethodInfo` constants; `EntityInsertBuilder`/`EntityUpdateBuilder`/`UpsertBuilder` walk the entity-Insert/Update/Upsert configure trees, PR #5482).
...
- **`InsertOrUpdate` and the fluent `Upsert` (PR #5482) are parallel, non-deprecated APIs.** `InsertOrUpdate` predates the fluent builder and is unaffected by it. `IUpsertSpec<T>`/`IEntityInsertSpec<T>`/`IEntityUpdateSpec<T>` follow the same declaration-location pattern flagged above for `IAsQueryableBuilder<TElement>` -- declared in `LinqToDB.Linq`, not `LinqExtensions/`, and never implemented (expression-tree-only receivers).
...
- **Entity Insert/Update/Upsert translation flow** -- `Insert(target,item,configure)` / `Update(target,item,configure)` / `Upsert(target,item,configure)` capture `configure` as `Expression<Func<ISpec,ISpec>>` (never invoked) -> `EntityInsertBuilder` / `EntityUpdateBuilder` / `UpsertBuilder` (EXPR-TRANS) walk the tree and synthesise the underlying `Insert`/`Update`/`Merge` shape. PR #5482 (issue #2558).
- **Eager-loading strategy markers** -- `WithUnionLoadStrategy`/`WithSeparateLoadStrategy`/`WithKeyedLoadStrategy` are runtime no-ops on `IQueryable<T>`; the outermost-applied marker wins over inner markers and over `LinqOptions.DefaultEagerLoadingStrategy`. PR #5450.
...
- `Source/LinqToDB/LinqExtensions/LinqExtensions.EntityInsert.cs` -- full read (105 lines) [this run, delta sha 36ee4f82; PR #5482]
- `Source/LinqToDB/LinqExtensions/LinqExtensions.EntityUpdate.cs` -- full read (105 lines) [this run, delta sha 36ee4f82; PR #5482]
- `Source/LinqToDB/LinqExtensions/LinqExtensions.Upsert.cs` -- full read (325 lines) [this run, delta sha 36ee4f82; PR #5482]
...
**Read (this run -- delta):**
- `Source/LinqToDB/LinqExtensions/LinqExtensions.EntityInsert.cs` (new file, sha 36ee4f82) -- entity-shaped `Insert`/`InsertAsync` via `IEntityInsertSpec<T>` fluent configure; delegates to `EntityInsertBuilder`. PR #5482 (issue #2558).
- `Source/LinqToDB/LinqExtensions/LinqExtensions.EntityUpdate.cs` (new file, sha 36ee4f82) -- entity-shaped `Update`/`UpdateAsync` via `IEntityUpdateSpec<T>`, PK-matched; delegates to `EntityUpdateBuilder`. PR #5482.
- `Source/LinqToDB/LinqExtensions/LinqExtensions.Upsert.cs` (new file, sha 36ee4f82) -- fluent `Upsert`/`UpsertAsync` family (`item` / `IEnumerable<T>` / `IQueryable<T>` sources) via `IUpsertSpec<T>`; delegates to `UpsertBuilder`; native single-statement upsert / synthesised `Merge` / emulated fallback per provider + `LinqOptions.UpsertEmulationPolicy`. PR #5482.
- `Source/LinqToDB/LinqExtensions/LinqExtensions.LoadWith.cs` (sha 36ee4f82) -- added eager-loading strategy marker region: `WithUnionLoadStrategy`/`WithSeparateLoadStrategy`/`WithKeyedLoadStrategy<T>` (PR #5450); `LoadWith`/`ThenLoad` XML docs now document the non-linq2db-source passthrough (fix for `InvalidCastException`, PR #5658). Rest of file (LoadWithAsTable, LoadWith/ThenLoad overload matrix) unchanged from prior run.
- `Source/LinqToDB/LinqExtensions/LinqExtensions.cs` (sha 36ee4f82) -- added named-filter overload `IgnoreFilters<TSource>(source, IEnumerable<string> filterKeys, params Type[] entityTypes)` mirroring EF Core 10's `IgnoreQueryFilters(IEnumerable<string>)`; an empty `filterKeys` list is a documented no-op that skips the `Expression.Call` injection (avoids query-cache/dedup churn). PR #5525. All other previously-documented members (SelectAsync, InsertOrUpdate, Drop, Truncate, Take/Skip/ElementAt, Having, NullsPosition ordering, Join family, CTE, AsQueryable two-arg, AsSubQuery, QueryName, InlineParameters, DisableGuard, HasUniqueKey, set operators, Tests/Tag/ToSqlQuery, AggregateExecute[Async]) unchanged at this sha.
