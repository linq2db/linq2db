...
- `LinqToDB.Common.Configuration` -- public static class of global flags and option-record proxies. The independently-mutable flat properties are `MaxBinaryParameterLengthLogging`, `MaxStringParameterLengthLogging`, `MaxArrayParameterLengthLogging`, `TraceMaterializationActivity`, `TranslationThreadMaxHopCount`, and `UseNullableTypesMetadata`; nested `Linq` / `Sql` / `RetryPolicy` / `BulkCopy` / `DataContext` / `Connection` / `QueryTrace` classes wrap the corresponding `*.Default` `OptionsContainer<T>` (`Source/LinqToDB/Common/Configuration.cs:24`, `:160`, `:529`, `:617`, `:743`). `OptimizeForSequentialAccess` (`:60`--`:68`) was converted from an independently-mutable field to a get/set proxy over `Linq.Options.OptimizeForSequentialAccess`, joining the `*.Default`-backed proxy pattern -- its setter now issues `Linq.Options = Linq.Options with { OptimizeForSequentialAccess = value }` instead of writing a standalone field; a `<remarks>` doc comment steers per-context callers to `DataOptionsExtensions.UseOptimizeForSequentialAccess` instead.
`Configuration.Linq` gained a matching new proxy, `PreferClientCalculation` (`:477`--`:485`), over `LinqOptions.PreferClientCalculation` -- when enabled, computed projection expressions (arithmetic, conditionals, unary ops, non-server-preferring mapped members/methods) are evaluated client-side during materialization instead of being pushed into SQL. Two additional nested classes exist outside the option-record pattern: `Configuration.SqlServer` (`:492`) -- single flag `UseSchemaOnlyToGetSchema` governing SQL Server schema-provider behavior -- and `Configuration.LinqService` (`:506`) -- two flags controlling remote-context type serialization (`SerializeAssemblyQualifiedName`, `ThrowUnresolvedTypeException`). `UseNullableTypesMetadata` (`:111`) uses the C# 14 `field` keyword in its setter to guard against enabling the flag on platforms where nullable metadata is unavailable (`Nullability.EnsureSupport()`).
...
- [DATA](../DATA/INDEX.md) -- `Configuration.RetryPolicy.Factory` returns an `IRetryPolicy?` consumed by `DataConnection`. `Configuration.BulkCopy.Options` is the default for `DataConnection.BulkCopy` (`Source/LinqToDB/Common/Configuration.cs:532`--`:560`, `:737`--`:741`).
...
- **`Obsolete("...in v7")` marker on retired flags.** `Configuration.IsStructIsScalarType`, `Configuration.UseEnumValueNameForStringColumns`, `Configuration.ContinueOnCapturedContext`, `Configuration.Data.ThrowOnDisposed`, `Configuration.Linq.PreloadGroups`, `Configuration.Linq.CompareNullsAsValues`, `Configuration.Linq.PreferApply`, `Configuration.Linq.KeepDistinctOrdered`, `Configuration.Linq.DoNotClearOrderBys`, and `Common.Array<T>` all carry `[Obsolete("...version 7"), EditorBrowsable(EditorBrowsableState.Never)]` plus a `// TODO: Remove in v7` comment on the line above (`Source/LinqToDB/Common/Configuration.cs:31`--`:33`, `:38`--`:40`, `:45`--`:47`, `:141`--`:143`, `:177`--`:179`, `:350`--`:356`, `:420`--`:422`, `:430`--`:432`; `Source/LinqToDB/Common/Array{T}.cs:10`--`:12`). The convention is consistent with CORE (see [CORE](../CORE/INDEX.md) -> "Recurring patterns").
- **Global-flag-as-`*.Default`-proxy migration.** `OptimizeForSequentialAccess` (`:60`--`:68`) and the new `PreferClientCalculation` (`:477`--`:485`) both carry an identical `<remarks>` block steering callers toward the per-context `DataOptionsExtensions.Use*` builder instead of the process-global flag (`Source/LinqToDB/Common/Configuration.cs:55`--`:59`, `:472`--`:476`). This applies the existing `Options`-wrapping shape (`Linq.Options`, `Sql.Options`, ...) directly to a flat property rather than routing through a nested class -- a template for retiring the remaining independently-mutable flags (`MaxBinaryParameterLengthLogging`, `MaxStringParameterLengthLogging`, `MaxArrayParameterLengthLogging`, `TraceMaterializationActivity`, `TranslationThreadMaxHopCount`) onto per-context options in a future version.
...
- Tier 2 (visited / total): 25 / 25 (100%) [ok]
...
- Read (this run -- delta):
  - Source/LinqToDB/Common/Configuration.cs -- `OptimizeForSequentialAccess` converted from an independently-mutable field to a get/set proxy over `Linq.Options.OptimizeForSequentialAccess` (`:60`--`:68`), with a new `<remarks>` block (`:55`--`:59`) steering per-context callers to `DataOptionsExtensions.UseOptimizeForSequentialAccess`; joined the `*.Default`-backed proxy pattern -- the prior "independently mutable flat properties" claim in `## Key types` was corrected to drop this member.
New property `Configuration.Linq.PreferClientCalculation` (`:477`--`:485`, remarks `:472`--`:476`) added, proxying `LinqOptions.PreferClientCalculation` -- controls client-side evaluation of computed projection expressions vs. SQL translation. All other previously-documented Configuration.cs line citations re-verified against the current file and shifted by the two insertions (+13 lines after the `OptimizeForSequentialAccess` rewrite at old line ~50, +22 more after the `PreferClientCalculation` insertion at old line ~453); corrected citations appear in `## Key types`, `## Inbound dependencies`, and `## Recurring patterns / known conventions`.
</details>
