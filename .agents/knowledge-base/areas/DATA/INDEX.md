...
Non-scalar object materialization (`CreateObjectReader<T>`, `:1868--1932`) now routes every `T` through `RecordReaderBuilder` unconditionally (`:1910--1914`) -- prior to this delta only types with `InheritanceMapping`/complex columns took that path, while plain records used a bespoke inline constructor-selection + `Expression.MemberInit` builder; the unification means raw-SQL `Query<T>`/`Execute<T>` now honor `[Column]` name mapping, `[ValueConverter]`, and constructor selection identically to `GetTable<T>()` (#4437, per the in-source comment at `:1910--1912`). `OptimizeForSequentialAccess` is now read from `dataConnection.Options.LinqOptions.OptimizeForSequentialAccess` rather than the static `Common.Configuration.OptimizeForSequentialAccess` (`:1925`).
...
- **`CreateObjectReader<T>`'s bespoke non-scalar reflection path is gone (this delta), superseded entirely by `RecordReaderBuilder`.** No known behavioral regression identified from reading the diff, but the removed path had handled anonymous/no-parameterless-ctor types via a constructor-parameter-name-match heuristic inline; confirm `RecordReaderBuilder` (LINQ area) covers that heuristic if a future raw-SQL materialization regression surfaces for constructor-bound anonymous-like types.
...
- Tier 1 (visited / total): 7 / 7 -- delta re-read: DataConnection.Configuration.cs at sha 4a478ff1, CommandInfo.cs at sha 36ee4f82
  ...
  - Source/LinqToDB/Data/DataConnection.QueryRunner.cs (partial)
  - Source/LinqToDB/Data/DataConnection.Interceptors.cs (partial)
  - Source/LinqToDB/Data/CommandInfo.cs -- delta re-read 2026-07-05; CreateObjectReader<T> non-scalar path unified onto RecordReaderBuilder (see Subsystems)
- Tier 2 (visited / total): 24 / 24 (100%)
- Tier 3 (skipped, logged): 0

Read (delta 2026-06-14, sha 4a478ff1):
...
Read (this run -- delta):
  - Source/LinqToDB/Data/CommandInfo.cs -- CreateObjectReader<T> non-scalar branch dropped the bespoke InheritanceMapping/HasComplexColumns-gated reflection path (manual constructor-parameter matching + Expression.MemberInit over td.Columns) and now unconditionally routes through RecordReaderBuilder for every non-scalar T, matching the LINQ query path entity construction (#4437 alignment note now literal in-source at :1910-1912); also OptimizeForSequentialAccess read moved from static Common.Configuration.OptimizeForSequentialAccess to per-connection dataConnection.Options.LinqOptions.OptimizeForSequentialAccess (:1925)
</details>
