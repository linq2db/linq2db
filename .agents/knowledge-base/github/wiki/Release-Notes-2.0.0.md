...
Starting from version 2.0, LINQ To DB will use ToString() method to create database value for enums, mapped to text column ([1006](/linq2db/linq2db/issues/1006), [1071](/linq2db/linq2db/issues/1071)). Prior versions used numeric representation of enumeration value, converted to string.
...
This release adds native support for common table expressions, including recursive CTE ([534](/linq2db/linq2db/issues/534), [890](/linq2db/linq2db/issues/890)). You can read more about this feature [here](https://github.com/linq2db/linq2db/wiki/CTE).

## Mapping

### `MappingSchema.EntityDescriptorCreatedCallback`

This new callback could be used to modify entity mapping descriptor after creation [(1074)](/linq2db/linq2db/issues/1074).
...
[961](/linq2db/linq2db/issues/961)
...
- complex types mapping fixed [(1005)](/linq2db/linq2db/issues/1005)

### Dynamic Columns

This new feature will allow you to use dynamic columns in your queries ([507](/linq2db/linq2db/issues/507), [744](/linq2db/linq2db/issues/744), [964](/linq2db/linq2db/issues/964),[1083](/linq2db/linq2db/issues/1083)). Check [this](https://github.com/linq2db/linq2db/pull/964) PR for more details.

### Calculated Columns

You can use expressions to define calculated columns using `IsColumn` property of `ExpressionMethodAttribute` attribute [(1004)](/linq2db/linq2db/issues/1004).
...
### Inheritance Mapping
- fixed incorrect query filter generation for left join associations for entities with inheritance mapping [(956)](/linq2db/linq2db/issues/956)
- fixed exception when inherited entity selected into property of base type [(1046)](/linq2db/linq2db/issues/1046)
- `Update`/`Delete`/`Insert`/`InsertOrReplace` extensions will properly recognize inherited values when passed as parameter of base type. Important: entities without inheritance mapping not affected by this change and query generation will use parameter type as before [(1017)](/linq2db/linq2db/issues/1017)
- `LoadWith` fixed to properly load derived entities [(994)](/linq2db/linq2db/issues/994)
- fixed several issues with type conversion in expressions between base and derived types ([1057](/linq2db/linq2db/issues/1057), [1065](/linq2db/linq2db/issues/1065))
- fixed exception when `LoadWith` called for nullable reference to entity with inheritance mapping [(996)](/linq2db/linq2db/issues/996)

### Other Changes and Fixes
- fixed `MappingSchema` converters were ignored for enums [(1006)](/linq2db/linq2db/issues/1006)
- fixed issue when adding new metadata reader to `MappingSchema.Default` could result in previously added readers being ignored [(1066)](/linq2db/linq2db/issues/1066)
- default `char` mapping will now use `Length = 1` [(1091)](/linq2db/linq2db/issues/1091)
- improved support of interfaces [(1099)](/linq2db/linq2db/issues/1099)
...
#### Other Changes and Fixes
- fixed exception generated for some cases when joins optimization enabled and table hints used [(949)](/linq2db/linq2db/issues/949)
- fixed SQL generation regression for some complex subqueries [(928)](/linq2db/linq2db/issues/928)
- fixed invalid SQL generated for empty select combined with `Take`/`Skip` [(817)](/linq2db/linq2db/issues/817)
- fixed issue when joined subquery condition were moved to outer query condition [(922)](/linq2db/linq2db/issues/922)
...
### New *Join extension methods ([1076](/linq2db/linq2db/issues/1076), [1088](/linq2db/linq2db/issues/1088))
...
You can add new query parameters from extension builders [(973)](/linq2db/linq2db/issues/973).
```cs
...
You can use it to preprocess IQueryable sources, passed to other APIs [(1116)](/linq2db/linq2db/issues/1116).
...
This delegate will be called after entity instantiation for contexts that implement `IEntityServices` interface [(1112)](/linq2db/linq2db/issues/1112). It allows you to pre-process or even replace created entity. `LINQ To DB` already implements this interface for default contexts: `DataContext`, `DataConnection` and `RemoteDataContextBase` classes.
...
Added trace callback for `DataContext` class similar to one that already exist on `DataConnection` class [(1131)](/linq2db/linq2db/issues/1131).
...
New [wiki page](https://github.com/linq2db/linq2db/wiki/Bulk-Copy) created for `BulkCopy` API. It still miss documentation on some options, but most important information is already here. It will be improved in next releases.

### KeepIdentity option

`KeepIdentity` option were documented and existing implementations tested/fixed to follow it [(1037)](/linq2db/linq2db/issues/1037).
...
### Other Changes and Fixes
- `CROSS JOIN` and `SelectMany` support in source query [(896)](/linq2db/linq2db/issues/896)
- `Merge` call will not be available anymore if you didn't specified any operations yet
- fixed exception of empty local source with source type != target type [(1153)](/linq2db/linq2db/issues/1153)
...
## Other Changes and Fixes
- errors during mapping of data from database to mapping class field on selects will now be wrapped into `LinqToDBException` with details what field failed with original error in `InnerException` [(1065)](/linq2db/linq2db/issues/1065)
- fixed issue when selected of `NULL` value using `SelectMany` method could have resulted in `default(T)` value for value types even with cast to T? [(1012)](/linq2db/linq2db/issues/1012)
- fixed exception passing binary data over WCF [(925)](/linq2db/linq2db/issues/925)
- spelling error fixed for `SchemaProvider.ForeignKeyInfo` [(941)](/linq2db/linq2db/issues/941)
- enforce server-side evaluation of `Sql.Lower`/`Sql.Upper` functions [(819)](/linq2db/linq2db/issues/819)
- fixed case when async code could be blocked on synchronous `Connection.Open` call [(1023)](/linq2db/linq2db/issues/1023)
- fixed support for type casts in `LoadWith` expression [(1069)](/linq2db/linq2db/issues/1069)
- fixed issue when insert query with sub-query data source will fail on next calls if it has nullable parameter and first call uses `null` for parameter value [(1098)](/linq2db/linq2db/issues/1098)
...
- T4 templates support in .NET Core projects [(1067)](/linq2db/linq2db/issues/1067)
- new `DataConnection.GetRegisteredProviders` method to return list of all registered data providers

# Provider-specific changes

## Access
- handle exceptions from OleDb provider on schema read calls when ACE provider used [(10)](/linq2db/linq2db.LINQPad/issues/10)
- schema provider will now return system tables too (`TableInfo.IsProviderSpecific == true`) [(1119)](/linq2db/linq2db/issues/1119)
...
### Other Changes and Fixes
- schema provider doesn't return procedures and functions if `GetTables = false` specified [(1068)](/linq2db/linq2db/issues/1068)

## Firebird

### Default identifier quotation mode change

Default identifier quotation mode changed from `FirebirdIdentifierQuoteMode.None` to `FirebirdIdentifierQuoteMode.Auto` [(1120)](/linq2db/linq2db/issues/1120).
...
### Other changes and fixes
- `BulkCopy` will throw exception if `KeepIdentity = true` option specified as this option is not supported for Firebird [(1037)](/linq2db/linq2db/issues/1037). Check [BulkCopy](https://github.com/linq2db/linq2db/wiki/Bulk-Copy) documentation for more details
- `DropTable` API will check if dropped objects exist before dropping them [(1120)](/linq2db/linq2db/issues/1120)
- fixed support of seconds and milliseconds by `Sql.DatePart` function [(967)](/linq2db/linq2db/issues/967)
- detect and escape identifiers that use reserved words in `FirebirdIdentifierQuoteMode.Auto` mode ([1095](/linq2db/linq2db/issues/1095), [1110](/linq2db/linq2db/issues/1110))
- `CreateTable`/`DropTable`/`TruncateTable` will respect identifier quotation mode during query generation [(1120)](/linq2db/linq2db/issues/1120)
- `FirebirdDataProvider` and `FirebirdSqlOptimizer` classes made public to help users override default implementation [(1000)](/linq2db/linq2db/issues/1000)
...
Schema provider for MySQL/MariaDB was updated to return procedures and functions [(991)](/linq2db/linq2db/issues/991). Requesting procedures and functions from transaction will throw exception because schema provider need to wrap it internally into transaction to avoid procedure execution due to bug in provider [(792)](/linq2db/linq2db/issues/792)
...
### Other changes and fixes
- `date` literal generation fixes [(969)](/linq2db/linq2db/issues/969)
- schema provider doesn't return procedures and functions if `GetTables = false` specified [(1068)](/linq2db/linq2db/issues/1068)
- detect and escape identifiers that use reserved words ([1095](/linq2db/linq2db/issues/1095), [1110](/linq2db/linq2db/issues/1110))

## PostgreSQL

### Native bulk copy support

Version 2.0 adds support for native bulk copy method [(935)](/linq2db/linq2db/issues/935). It is available through existing `BulkCopy` LINQ To DB API by specifying `BulkCopyOptions.BulkCopyType = BulkCopyType.ProviderSpecific`.
...
### Upsert support
`InsertOrUpdate` API will use `INSERT ON CONFLICT UPDATE` statement for PostgreSQL 9.5+ instead of several statements on previous versions [(948)](/linq2db/linq2db/issues/948).

You will need to use `PostgreSQLVersion.v95` provider version if you don't use version autodetect.

### Other changes and fixes
- improved support for some database types as a part of `BulkCopy` improvements [(1091)](/linq2db/linq2db/issues/1091)
- fixed support for following types in `CreateTable` API: `Int16`/`Int64` identity columns, `System.Linq.Binary`, `DataType.VarBinary`, `DataType.NChar(1)`, `char` [(1091)](/linq2db/linq2db/issues/1091)
- support for interval type mapping to both `NpgsqlTimeSpan` and `NpgsqlInterval` types [(1091)](/linq2db/linq2db/issues/1091)

## SAP HANA

### `BulkCopy` `KeepIdentity` option support

As a part of `KeepIdentity` option review, support for it added to SAP HANA provider. Check [documentation](https://github.com/linq2db/linq2db/wiki/Bulk-Copy#keepidentity-option-default--false) for more details [(1037)](/linq2db/linq2db/issues/1037).
...
### `BulkCopy` `KeepIdentity` option support
As a part of `KeepIdentity` option review, support for it added to SQL CE provider. Check [documentation](https://github.com/linq2db/linq2db/wiki/Bulk-Copy#keepidentity-option-default--false) for more details [(1037)](/linq2db/linq2db/issues/1037).

## Sybase
- `MERGE` insert operation will respect `SkipOnInsert` on identity fields when `InsertWhenNotMatched()` without custom setter used and allow database to generate field's value [(914)](/linq2db/linq2db/issues/914)
- requesting procedures and functions from schema provider will throw exception if called from transaction to avoid database corruction due to bug in provider [(792)](/linq2db/linq2db/issues/792)

## SQLite
- `DateTime.AddDays()` method to `SQL` conversion fixed [(998)](/linq2db/linq2db/issues/998)

## SQL Server
- `varchar`/`nvarchar` parameters will use 8000/4000 as length to improve query plans caching [(989)](/linq2db/linq2db/issues/989)
- `MERGE` will use parameters instead of literals for binary data in client-side (IEnumerable) source
- `MERGE` insert operation will respect `SkipOnInsert` on identity fields when `InsertWhenNotMatched()` without custom setter used and allow database to generate field's value [(914)](/linq2db/linq2db/issues/914)
- legacy `MERGE` API will not try to update identity columns anymore on update operation anymore [(1007)](/linq2db/linq2db/issues/1007)
- fixed `DropTable` method not dropping table in another database [(1030)](/linq2db/linq2db/issues/1030)
- fixed DateTime literal generation [(1107)](/linq2db/linq2db/issues/1107)
- fixed incompatibility between `BulkCopy` and `RetryingDbConnection` [(1135)](/linq2db/linq2db/issues/1135)

# Changes for developers
- Query AST was refactored. See `SelectQuery` class ([936](/linq2db/linq2db/issues/936), [938](/linq2db/linq2db/issues/938))
- Tests configuration changed format and use *DataProviders.json files instead of *DataPRoviders.txt. [More details](/linq2db/linq2db/blob/master/CONTRIBUTING.md#configure-data-providers-for-tests)
...
- `SchemaProviderBase` methods `ToTypeName` and `ToValidName` made public ([944](/linq2db/linq2db/issues/944), [963](/linq2db/linq2db/issues/963))
- `SqlProviderFlags.CustomFlags` list added to allow store custom provider flags [(1154)](/linq2db/linq2db/issues/1154)
- expose `SqlExtensions` class for provider developers

# I Use Entity Framework (:feelsbadman:)

Don't wory, check this [new project](https://github.com/linq2db/linq2db.EntityFrameworkCore) we created recently. It is still an early prototype so don't expect it to work flawlessly. We will appreciate your feedback!