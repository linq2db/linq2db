...
### Two convert passes — statement-scoped visitor state doesn't survive to build time

`SqlExpressionConvertVisitor` runs **twice**: once over the whole statement in `OptimizationContext.OptimizeAndConvertAll` (`visitQueries: true`, so it descends into every subquery), and again **per select-clause at SQL-build time** via `BasicSqlBuilder.BuildColumns` → `ConvertElement` (`OptimizeAndConvert`, `visitQueries: false`) — which converts each `SelectQuery.Select` in isolation, with no enclosing-statement context. Consequence: a provider convert visitor that keys behavior off statement context — e.g. an override of `VisitSqlInsertStatement` / `VisitSqlMergeStatement` that sets a flag read by `WrapColumnExpression` — reaches MERGE sources and `INSERT … SELECT` (converted in the full-statement pass) but **not** a scalar subquery embedded in an `INSERT … VALUES` / a setter, which is (re)converted standalone at build time. Don't assume statement-visit state is available when a column expression is converted. (Surfaced on the Firebird 6 `UUID_TO_CHAR` read-wrap: a modification-statement guard fixed MERGE/INSERT-SELECT Guid writes but couldn't reach the INSERT-setter subquery.)
