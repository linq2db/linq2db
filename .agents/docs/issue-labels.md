...
- Set types via `gh api -X PATCH … -f type=…`; add/remove labels via `gh issue edit <n> --add-label … --remove-label …`; manage label *definitions* via `gh label create/edit/delete --yes`. Wrap multi-issue loops in a `.build/.agents/*.ps1` script (one allowlist match, not N).
