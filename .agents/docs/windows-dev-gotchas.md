...
- **Recovery / reference — the raw plumbing the script wraps.** Rebuild the commit so the working tree's missing colon page is never consulted (run as separate calls — the Bash tool doesn't persist shell vars, so capture each printed SHA and substitute it into the next call):
  ...
  **`update-index` needs `-c core.protectNTFS=false` too whenever the path it touches is the colon one** (adding it, or `--force-remove`-ing it in a rename). Without the flag it does not fail — it prints `Ignoring path <page>` and **exits 0**, so the index silently keeps the old entry and the commit you build from it is wrong. The guard is `verify_path()`, not sparse-checkout: `-c core.sparseCheckout=false` changes nothing, and neither does `--ignore-skip-worktree-entries`. Renaming the colon page therefore needs both halves flagged (`--add --cacheinfo` for the new path, `--force-remove` for the old), and `ls-files` with a `':(literal)…'` pathspec to confirm — a bare `[Internal]…` pathspec is glob-parsed as a character class. (Cost two silently-ignored calls plus a wrong sparse-checkout hypothesis on 2026-07-31 doing the rename above.)
...
**Renaming a wiki page** (e.g. a diagnostic id changed on the PR): with the clone in its normal full-checkout state, plain `git mv <old>.md <new>.md` + `git add` + commit works. The rest of this paragraph applies only to a clone still in the sparse fallback state above — there, `git mv --sparse <old>.md <new>.md`, since plain `git mv` fails with a "Use the --sparse option" hint because the destination isn't in the sparse set. Then edit + `git -C ../linq2db.wiki add --sparse <new>.md` (the `--sparse` is likewise required), commit, push. Also `git -C ../linq2db.wiki grep -n "<oldId>" HEAD -- "*.md"` first to catch any *other* page linking the old name (the rename would break those links). Done on PR #5703 (`LINQ2DB1001.md` → `L2DB1001.md`, plus its `helpLinkUri`/editorconfig id references).
