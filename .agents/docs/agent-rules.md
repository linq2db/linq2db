...
This is the **Claude-Code-specific operational overlay**, auto-imported by `CLAUDE.md`. The shared, agent-agnostic contributor rules live in [`AGENTS.md`](../../AGENTS.md) at the repo root (also read natively by Codex and referenced by Copilot); this file layers Claude Code harness mechanics (shell/tool rules, permission patterns, worktrees, `.agents/` curation, subagent verification, skill workflows) on top, and is the detail source for several topics `AGENTS.md` summarizes. Where a rule here is genuinely agent-agnostic it is also stated more briefly in `AGENTS.md` — the canonical statement is whichever the agent is reading; they must not contradict.
...
**Markdown tables must stay contiguous.** When authoring or editing a markdown table (docs, PR/issue/review bodies, this corpus), never insert a paragraph, note, or blank-line-then-prose **between** table rows — it terminates the table and the rows below render as a second, header-less table. Put explanatory notes above or below the whole table, not in the middle. (Surfaced when a parenthetical note placed between rows split the PR #5468 capability-matrix table in two.)
...
When `.agents/knowledge-base/` is present, it's the curated synthesis over issues, PRs, git history, and per-area code patterns — cheaper than re-deriving the same context from raw `gh` / `git` / source. Reach for it **first** for "why is it this way" / past-decision questions, bug investigation (a known issue or prior fix may already be recorded), and feature / area orientation. Two ways in, **cheapest first**:

- **Read directly — the default.** `areas/<AREA>/{INDEX,issues,decisions,patterns,tech-debt}.md` for a subsystem, `architecture/*.md` for the pipeline, `history/decisions/<slug>.md` for ADR-equivalent records, `glossary.md` for terms (area codes in [`kb-areas.md`](kb-areas.md)). A targeted `Read` / `Grep` — or `kb-search.ps1` for a keyword sweep across the KB — answers most orientation questions for the cost of a file read. Make this your reflex.
- **`/kb-ask <question>` — synthesis only.** KB-grounded Q&A with citations, but it spawns the `kb-research` subagent, so reserve it for questions that span multiple areas or need a synthesized answer — not for "what does `areas/PROV-ORACLE/issues.md` say" (just read it).
...
Before proposing code changes for a bug fix or new feature, do two cheap orientation passes first. **(1) Consult the KB for the affected area** — read `areas/<AREA>/{issues,decisions,patterns,tech-debt}.md` directly (area codes in [`kb-areas.md`](kb-areas.md); reserve `/kb-ask` for cross-area synthesis), since a prior decision, known issue, or idiomatic pattern may already shape the fix (see *Consult the knowledge base* above). **(2) Enumerate existing tests** that already exercise the affected path and surface them to the user — Grep `Tests/` for the target code's keywords (SQL builder type, translator method, provider class); shortlist `<Fixture>.<Test>` entries with a one-line purpose each; flag what the new work adds on top. Do both before writing code and before invoking `test-writer`. When there's no obvious affected code path yet (greenfield feature, vague bug report), say so and ask the user to narrow the target first.

**Once the user has chosen "fix" over "gate", keep digging to the root.** Gating / `[ActiveIssue]` / provider-exclusion is a fallback, not a recurring offer. After the user has explicitly asked to fix a failure rather than gate it, don't resurface "shall I just gate it?" every time the investigation gets deep or multi-layered — drive to the actual root cause (instrument, reproduce minimally, follow the user's debugging hints) and return to gating only if you can *demonstrate* the fix is infeasible or genuinely out of scope. Repeatedly proposing the gate after a fix was requested reads as giving up. The same anti-pattern applies to offering to **stop or hand off to "the author"** mid-investigation: when the user is driving and says *dig / continue / do it yourself*, exhaust empirical investigation first — build the relevant baseline (e.g. a fresh `origin/master` worktree) to anchor a divergence, instrument with probe `throw`s / logging, attempt-then-test candidate fixes — before proposing to defer. (Reverting an unproven cross-cutting change is still correct; persistence means driving the investigation to ground, not merging a risky fix.)

And the mirror discipline when **un**-gating after a fix lands: verify **every** gated provider, don't leave a gate in place on the *assumption* that a harder-to-reach provider stays broken. A netfx-only / file-DB provider (SqlCe, Access) is still locally runnable (`dotnet test -f net462 --provider …`); reach for it and confirm pass/fail rather than keeping its gate "to be safe". (Corrected when a SqlCe `#5599` gate was left in place by assumption while the YDB sibling was verified and ungated — the fix resolved both.)

**Prefer the least-invasive resolution, and verify before asserting.** When a reported bug might be a usage problem, exhaust the lighter fixes first — an existing/built-in API (e.g. `??`/COALESCE over a custom `[Sql.Expression]`), a `Sql.Extension` builder that emits a real AST node, or registering a custom / third-party scalar type with the mapping schema (`SetScalarType` / `SetDataType`) — before proposing a change to cross-cutting core (SQL AST, `QueryHelper`, translators; see [`agent-guardrails.md`](agent-guardrails.md) → *Don't reshape cross-cutting internals*). And never claim a fix works on static reasoning alone — back it with a reproduced red→green test or a CI run. A core change proposed without empirical proof is the "too optimistic" failure mode. When the fix addresses a **review finding** (yours or a reviewer's), define the **red** regression test *first* — one that fails on the current behaviour and passes after the fix — before writing the fix, whenever the failure is reproducible; the failing test is what confirms the finding is real and that the fix targets it (on PR #5643 the failing `OutputSupportSurface` / ClickHouse-refresh tests *were* the red proof that turned two "minor" review notes into demonstrated defects). **If the red test can't reproduce the finding, don't drop it silently and don't fix on speculation** — reframe it as an FYI to the PR author stating you could not reproduce a defect, with the repro details and any test that pins the current (correct) behaviour. (On PR #5604 a VALUES-dedup type-divergence suggestion that no strict-typed-provider repro could trigger — the column's string use was client-side, so the placeholder kept its own type — was posted as a "could not reproduce" note plus a regression test pinning the behaviour, rather than a speculative `EnumerableContextDynamic` change.) And when authoring code that builds SQL, never interpolate a user-supplied value into a SQL string — route it through a parameter or a `Sql.*` / AST builder that parameterizes. linq2db is a SQL-generating library, so a string-concatenated value is the SQL-injection class by construction, and AI-generated SQL is a documented source of exactly this flaw.
...
- **Tracing when a code token / heuristic was introduced** — use `git log -S "<token>" --all -- "*<Filename>"` (filename glob, not full path); path-scoped `-S` and even `--follow` miss the lineage when the file moved namespaces, surfacing only the rename commit.
...
Pairs with *Issue-proposed fix details are written from memory — verify them* above (untrusted **content** is this rule; unreliable **claims** is that one) and the skill-file supply-chain note in [`maintaining-the-corpus.md`](maintaining-the-corpus.md) → *Editing skill / hook / agent files is a supply-chain surface*.
...
When iterated `dotnet build` / `/test` / `/release-verify` runs fail with `Access to the path '<dll>' is denied` (build-server file lock — fix: `dotnet build-server shutdown`) or `MSB3021/MSB3027 not enough space on disk` (`.build/bin/` accumulation — fix: `Remove-Item -Recurse -Force .build/bin .build/obj`), see [`windows-dev-gotchas.md`](windows-dev-gotchas.md) → *Iterative-build gotchas*.
...
Any skill, subagent, or ad-hoc command that needs to write a scratch file (JSON payloads for `gh api --input`, generated diffs, intermediate output, etc.) must place it under **`.build/.agents/`** at the repo root.

- `.build/` is already in `.gitignore`, so files there are never accidentally committed.
- The `.agents/` subpath disambiguates from other tooling's build output.
- Ensure the directory exists with `mkdir -p .build/.agents` (single Bash call) before the first write in an invocation.
- Name files so they identify the skill and any relevant id, e.g. `.build/.agents/review-pr-1234.json`.
...
- **HTTP 422 "internal error" from `gh api`** = transient GitHub-side outage on that endpoint. Report once with in-flight context, preserve scratch under `.build/.agents/`, wait for user direction. Don't retry-loop, don't poll `githubstatus.com`.
- **`gh api -f body=@<file>` does NOT read the file** — it stores the literal `@<file>` as the body. Same trap as `gh … --body @-` (banned in [`windows-dev-gotchas.md`](windows-dev-gotchas.md)). Use `--input <json-file>` with a pwsh-built wrapper, or `-F body=@<file>` (capital F) for the specific endpoints where gh CLI documents field coercion.
...
The most-violated guardrails stay inline. Lower-traffic guardrails (Surface trade-offs, Build configurations `==` vs `!=`, Document arbitrary values, BOM after `Write`, Default to script over hook, Provider / Codebase claim verification, Distinct lenses for parallel reviewers, Cross-model verification for cross-cutting core, Persona framing, Editing whitespace / control characters) live in [`agent-guardrails.md`](agent-guardrails.md). Codebase design invariants these protect — public-API contract, cross-cutting internals, SQL AST namespace placement, column-aligned formatting — live in [`code-design.md`](code-design.md).
