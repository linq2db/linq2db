...
For those cases, emit a BLK whose **Fix** is *"Move `<TypeName>` to `LinqToDB.Internal.SqlQuery` as part of this PR — the signature change then becomes AST-internal and no longer trips ApiCompat."* — not the milestone-gate wording above. The AST nodes still under `LinqToDB.SqlQuery` are `SqlFrameBoundary`, `SqlFunctionArgument`, and `SqlWindowOrderItem`, each carrying a `// TODO: v7 - move to internal namespace` marker. (`SqlFrameClause`, `SqlExtendedFunction`, and `SqlSearchCondition` were cited here historically but have **already moved** to `LinqToDB.Internal.SqlQuery` — don't use them as examples. Re-derive the current list with `git grep -l "TODO: v7 - move to internal namespace" -- "Source/LinqToDB/SqlQuery/*.cs"` (grepping the namespace instead returns the whole `LinqToDB.SqlQuery` set — enums and helpers that legitimately stay — not just the movable AST nodes) rather than trusting this sentence, which goes stale with each move.) When in doubt, check whether the type is referenced only from query-building / translation / SQL-rendering code — if yes, it's AST and belongs internal.
...
- **Interface-member additions are binary-breaking but still classify as `added`.** Adding a member to a shipped public interface (e.g. `IDataProvider`) is source-additive but *binary*-breaking for third-party types that implement the interface directly — they fail to load with `TypeLoadException` until recompiled — and ApiCompat records it as a `CP0006` suppression in `CompatibilitySuppressions.xml`. This does **not** change the classification: it stays `added` and is allowed at any milestone per the additive rule above. Do **not** escalate an interface-member addition to BLK/MAJ, or demand deferral to the next major, on the strength of the `CP0006` suppression alone — note it for release notes (downstream provider authors must recompile) and move on. (Confirmed on #5600: `IDataProvider.CreateParameter` added in the 6.4.0 minor, accepted via `CP0006` + `[EditorBrowsable(Never)]` binary-compat record overloads.)
...
- **Drop any `PublicAPI.Shipped.txt` / `PublicAPI.Unshipped.txt` drift finding outright.** [`code-reviewer.md`](../agents/code-reviewer.md) already forbids the passes from emitting these (*"Never emit findings about these files"*), but a pass can still slip one through (e.g. "new public surface absent from `PublicAPI.Unshipped.txt`"). The orchestrator must discard it at classification time — do not promote it to the review, the interactive walk, or a fix. These files lag source **intentionally** inside a release cycle: `RunApiAnalyzersDuringBuild` is opt-in (default `false`, and off in CI's `build` leg, so RS0016/RS0017 do not gate), and at HEAD they can be **hundreds of symbols stale** (a `-p:RunApiAnalyzersDuringBuild=true` build on #5525 surfaced ~1368 unrecorded pre-existing public symbols — DuckDB provider, `ConflictAction`, etc.). ApiCompat / `CompatibilitySuppressions.xml` (step 1) is the authoritative additive-surface check, and purely additive public API needs **no** Unshipped entry. Never build with `-p:RunApiAnalyzersDuringBuild=true` to "verify" such a finding — the rule is categorical, and the release PublicAPI step (`/release-publicapi`) owns the pre-release refresh.
