...
param(
    ...
    [string]$StatePath        # optional override; default = .build/.agents/release-<ver>.json
)
...
function Write-StateFile {
    ...
    # Always UTF-8 without BOM (matches the rest of `.build/.agents/` and avoids
...
function Set-PrBody {
    ...
    $tmp = Join-Path (Get-Location) ".build/.agents/release-state-prbody-$PrNumber.md"
