...
Action:
  ...
  diff      Fetch + merge per-package across TFMs. Inputs:
              ...
              [-WriteDir <path>]       # raw-HTML cache dir (default .build/.agents/fuget-<run>/)
...
# Run-cache dir for raw HTML responses (gitignored under .build/.agents/).
if (-not $WriteDir) {
    $stamp = Get-Date -Format 'yyyyMMdd-HHmmss'
    $WriteDir = Join-Path (Get-Location) (Join-Path '.build/.agents' "fuget-$stamp")
}
