...
5. **Playground link (optional).** When the caller passes `playgroundLink: true`, after the main edit add a `<Compile Include="..\Linq\<relative>.cs" Link="<Name>.cs" />` item to `Tests/Tests.Playground/Tests.Playground.csproj`, placed alphabetically under the existing `<Compile Include=... />` entries in the same `<ItemGroup>`. Report the csproj edit as an additional `files[]` entry with `testKind: "playground-link"`. If the caller omits this flag, do not touch the csproj. **Leave the link in place when you return** — `/test` and `test-runner` require it to be present to run the playground project, and reverting it here would abort the very run the caller asked for. The main-project test file stays in place permanently; only the `<Compile Include>` line in `Tests.Playground.csproj` is transient, and **removing it is the caller's end-of-iteration responsibility** (before staging — leaving it in would pollute the commit; see [`agent-rules.md`](../docs/agent-rules.md) → *Never commit playground scratch*).
