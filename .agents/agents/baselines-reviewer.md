...
3. **Read `sizeOutliers[]` and `regressionCandidates[]` before pattern grouping.** These two surface tests whose shape regressed in ways the dominant-pattern view hides:
   ...
   **Duplicate-value parameter split (failed dedup).** When the `+` side **adds** a `DECLARE @<base>_<n>` whose `SET` value equals an already-declared `@<base>` (suffixed name, *same value*), the diff is one logical parameter split into per-occurrence copies — rendered under two names with duplicated DECLARE/SET. This is the fingerprint of a **failed** parameter deduplication (a convert/wrap visitor that clones a parameter at some occurrences but leaves the original at others — e.g. cast vs non-cast positions), **not** a benign cast-removal. Classify as `changed_suspect` naming the split, even when the PR's stated intent is "remove spurious casts" — the cast removal and the parameter split are separate effects, and the extra DECLARE inflates the parameter count on providers with hard limits (Firebird, DB2). Do **not** fold `@X` + `@X_1` (same value) into a "one reused clone" / `changed_expected` note. (Surfaced on PR #5723: `MergeTests.TestParameters1(Firebird.3)` split `@Val5` → `@Val5` + `@Val5_1`; `UpsertTests.Single_ConditionalUpdate_When(DB2)` split `@Version` → `@Version` + `@Version_1` — duplicate values, mislabeled benign.)
...
5. Scan for **unusual cross-provider distinctions**. Routine syntactic variation (parameter prefixes, identifier quoting, paging syntax, boolean rendering — see the layout doc) is expected and must not be called out. Call out: structural shape differences, one provider emitting extra/fewer joins, one provider emitting a subquery others don't, one provider missing a WHERE clause, etc.
   ...
   - **Subset baseline coverage can be a failure fingerprint, not just scope.** When a *new* test emits baselines for only some of the providers its `[IncludeDataSources]` list targets, the absent providers may mean the test **errored** there (no baseline is captured when a test fails), not that it intentionally excluded them. `Grep` the test source for its `[IncludeDataSources(...)]` set; if baselines are missing for providers that set covers, call it out as a **possible test failure** on those providers rather than "expected shape" — especially when the PR's CI shows failing legs for exactly those providers (the review skill checks CI per [`review-pr`](../skills/review-pr/SKILL.md) → step 2c). Surfaced on PR #5678: `ProviderSpecificReaderValueTests` produced baselines only for the providers it passed on; Oracle/PostgreSQL/DuckDB/DB2 folders were absent because the test errored there — a CI blocker, not a coverage choice.
