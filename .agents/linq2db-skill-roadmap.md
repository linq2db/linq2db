...
| [x] | CTE | `Source/Skills/linq2db/docs/query-cte.md` | High | Done | `.AsCte()` and recursive CTE patterns; audited against source, no defects found (`AsCte`, `GetCte`, `InnerJoin` predicate form, `SelectQuery`, `Concat`/`UnionAll` equivalence all confirmed accurate). |
...
| [x] | Translatable methods | `Source/Skills/linq2db/docs/translatable-methods.md` | Medium | Done | String, Math, DateTime translation guidance. Audited against source: the "authoritative" `Sql.*` table listed 4 non-existent methods (`Sql.In`, `Sql.IsNull`, `Sql.Coalesce`, `Sql.Exists`) and omitted ~25 real ones - fixed, added real equivalents, added missing common members, cross-linked `null-semantics.md`/`parameters.md`, corrected misleading "where to verify" file pointers. Math rounding-mode claims, DateTime tables, Parse→`ConvertTo<T>` claim, and string-concat null semantics all confirmed accurate. |
| [x] | Extension mechanisms | `Source/Skills/linq2db/docs/extensions.md` | High | Done | SQL function/expression mapping plus `IMemberTranslator` member translators (moved here from `configuration.md`). Audited against source, no defects found - `[Sql.Expression]`/`[Sql.Function]`/`[ExpressionMethod]` properties, `ISqlExtensionBuilder.Add`/`Concat`, and `[SqlQueryDependentParams]` deprecation all confirmed accurate. |
| [x] | Interceptors | `Source/Skills/linq2db/docs/interceptors.md` | Medium | Done | Callback choice and registration. Audited against source, no defects found - all 7 interceptor interfaces and every documented method/behavioral nuance confirmed accurate word-for-word against XML doc comments. |
...
| [x] | Joins - translation gotchas | `Source/Skills/linq2db/docs/query-joins.md` | High | Done | Not a LINQ-join syntax tutorial. Fluent `InnerJoin`/`LeftJoin`/`RightJoin`/`FullJoin`/`CrossJoin`, association-driven implicit joins, composite-key mismatch translation failure, redundant-join optimization, and `RightJoin`/`FullJoin`/`APPLY` provider limitations (unvalidated at the LINQ layer). |
