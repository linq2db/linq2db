...
4. **Cross-provider anomalies** under `### Cross-provider anomalies`, one bullet per entry.
   ...
   **Scan the whole diff programmatically for structural signals — the grouped summary is sample-based and under-reports.** The subagent characterises 700+ files from sampled pattern-groups, so a change shape that spans groups it didn't sample is described narrowly or missed. These whole-diff scans are cheap and have each paid for themselves; run them before writing the Baselines section:
