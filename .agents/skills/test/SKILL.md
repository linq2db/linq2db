...
- Do not launch multiple test processes to cover several providers. The harness parallelizes across providers **in-process**, so pass the full set to a **single** `test-runner` invocation (`--provider a,b,c`) and let it parallelize internally; a second concurrent process only risks shared-database corruption, not more coverage (see `.agents/docs/testing.md` → **Testing several providers in one run**). Don't fan out multiple `test-runner` invocations with overlapping target sets.
