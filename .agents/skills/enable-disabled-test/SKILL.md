...
- **Branch + commit + push rules**: `.agents/docs/agent-rules.md` → *Creating a new branch*, *Git commit rules*, *Push to remote rules*, *Pull request rules*
- **Enabling-test guardrail**: `.agents/docs/bug-investigation.md` → *Enabling an `[ActiveIssue]` test after the issue closes*
- **Worktree mechanics for bisect**: `.agents/docs/worktree.md`
- **Bisect-across-SDK gotcha**: `.agents/docs/windows-dev-gotchas.md` → *Bisecting across SDK upgrades*
- **Closed-milestone PATCH**: `.agents/docs/github-authoring.md` → *`gh issue edit --milestone` rejects closed milestones*
- **`/test` skill** (used to verify pass on master): `.agents/skills/test/SKILL.md`
- **`pr-resolver.md`** (issue → PR lookup): `.agents/docs/pr-resolver.md`
...
4. **Loop.** For each candidate commit:
   ...
   - `dotnet test --project <project> --filter "FullyQualifiedName~CreateData.CreateDatabase|FullyQualifiedName~<TestClass>.<TestMethod>" -c Debug --settings .runsettings -p:TreatWarningsAsErrors=false -p:NoWarn=CS9336` — the `TreatWarningsAsErrors=false` + `NoWarn` flags are needed when bisecting across SDK upgrades (see [`windows-dev-gotchas.md`](../../docs/windows-dev-gotchas.md) → *Bisecting across SDK upgrades*); extend `NoWarn` with extra IDs as historic-code warnings surface.
