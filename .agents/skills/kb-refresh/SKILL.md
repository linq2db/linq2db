...
- **`next_cursor` is safe to use directly.** `kb-fetch-github.ps1` normalizes both the `since` input and the `next_cursor` output through `To-IsoString` (script lines 61-70), so the cursor round-trips as `yyyy-MM-ddTHH:mm:ssZ`. The old workaround — "next_cursor is locale-formatted (`07/06/2026 14:54:08`), compute `max(item.updated_at)` yourself" — is obsolete; don't hand-compute the max.
