...
- Queries `https://api.nuget.org/v3/registration5-gz-semver2/<id>/index.json` for each unique package — returns **listed** versions only (skip unlisted — they may be removed/deprecated). Parallel fan-out, cached at `.build/.agents/release-<ver>-deps-cache/<id>.json`.
- Applies policy:
