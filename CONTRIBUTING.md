...
**Restart your agent after populating the submodule.** Claude Code reads skills, subagents and the `CLAUDE.md` import set once at session start, so a session that was already open while `.claude/` was empty stays empty-handed even after `git submodule update --init` succeeds — the files are all on disk and the session still resolves no skills and no project instructions.
