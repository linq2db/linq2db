...
2. **Compose the user-facing text.** Read the code diff (`diff-reader.ps1`), the PR body, and linked issues/discussions. Write:
...
4. **Write the comment.** Build the manifest at `.build/.claude/release-notes-draft-<pr>.json`:
   ```json
...
1. `release-notes-draft.ps1 -Action sweep-plan -Milestone <ver> [-PlanFile .build/.claude/release-<ver>-notes-plan.json] [-WikiClone <path>]`.
...
4. Once drafts exist, build the cumulative `apply-wiki` manifest from `harvest` and run the `apply` procedure once (diff → confirm → push).
5. Hand off to `/release-notes-validate` for the final coverage confirmation.

### Mode: `harvest` (assemble the GitHub-release brief)

1. `release-notes-draft.ps1 -Action harvest -Milestone <ver> [-PlanFile ...]` → `.build/.claude/release-<ver>-notes-harvest.json` (`items[]` with per-PR `omit` / `includeBrief` / `fullText` / `briefText`).
