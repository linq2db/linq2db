...
| Context budget (always-loaded set) | `audit-claude` | total bytes of `CLAUDE.md` + every doc reachable via the `@import` chain from it (currently `CLAUDE.md` + `.claude/docs/agent-rules.md`). ≥ 90 KB → 🔴, ≥ 60 KB → 🟡. Every conversation pays this on startup; hands off to `audit-claude` (its refactor-candidate check proposes the section relocations). |
