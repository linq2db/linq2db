...
- **Source-file encoding / BOM claims.** When a bot claims the PR added or removed a file's UTF-8 BOM (or changed its `charset`), read the cached `_diff/<path>.diff` — the per-file diff preserves the BOM marker in its context lines (a removed BOM shows as `-﻿using …` → `+using …`), so it answers the claim directly. Do **not** `xxd` / `head -c` the cached HEAD body: the cache writer may normalize encoding (stripping the very BOM you're checking for), and the raw byte-pipe prompts on the allowlist. Surfaced 2026-05-31 on PR #5542 (a Copilot BOM-removal claim confirmed Still-actual from the `_diff`, after a redundant `xxd` pass). **Once the `_diff` confirms an add/remove, classify it against the repo convention rather than re-checking bytes:** `.editorconfig` sets `charset = utf-8-bom` for `.cs` / `.csx` / `.vb` / `.vbx` (see [`agent-guardrails.md`](agent-guardrails.md) → *Prepend the UTF-8 BOM after `Write`*), so for those file types a BOM **addition** conforms to convention (a bot flagging it as scope-creep is **Inaccurate**, not a finding) and a BOM **removal** deviates (a legit finding). Settle the convention from `.editorconfig charset`, never an `od` / `head -c` / `git show … | od` pass over a sibling file — that re-derives what `.editorconfig` already states and the byte-pipe prompts on the allowlist. Surfaced 2026-05-31 on PR #5525, where the PR's BOM additions (`QueryFilterAttribute.cs` plus two new files) were correct per `.editorconfig`, confirmed only after a redundant `git show … | od` that `.editorconfig` had already answered.
