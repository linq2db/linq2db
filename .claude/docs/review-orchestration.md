...
- **`code-reviewer` briefing** (one per pass when multi-pass)
  ...
  - `writeDir: .build/.claude/pr<n>` — mandatory on the first `diff-reader.ps1` call so full file bodies land on disk for `Read` / `Grep` navigation. **When the session runs in a git worktree, also spell out the absolute worktree-prefixed cache root in the briefing** (e.g. `<worktree-root>/.build/.claude/pr<n>`): subagents that guess the main-repo prefix get permission-denied on `Read`/`Grep`, burn calls rediscovering the path, and in the worst observed case fell back to degraded `Grep`-only access that produced a false finding (PR #5450 review, 2026-06-12).
