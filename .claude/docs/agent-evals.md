...
Small, fast, deterministic-where-possible checks — runnable as a smoke suite (target ~20–50 cases), per change to `.claude/`:

1. **Script contract / schema.** For each `.claude/scripts/*.ps1` with a JSON-in / JSON-out contract (`diff-reader.ps1`, `verify-lines.ps1`, `pr-context.ps1`, …), feed a fixture input and assert the output shape (required keys, types) matches the documented schema. Pure mechanical, no model.
...
- **CI integration.** Whether this runs in the linq2db CI (it touches no product code) or as a local `/`-skill the curator runs before landing `.claude/` changes on `infra/claude-curation`.
...
Until built, the existing safeguards stand in for this: `git status` verification after every subagent ([`agent-rules.md`](agent-rules.md) → *Agent guardrails*), the `/audit-claude` static checks, and human review of `.claude/` edits.
