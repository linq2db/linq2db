...
- `Source/LinqToDB/Internal/Linq/Builder/ExpressionBuilder.Aggregation.cs` -- BuildArrayAggregationFunction (new method for captured-collection aggregates, PR #5557); WrapAggregateResult static helper (eager SqlRewriter for grouped, deferred for non-grouped); AggregationContext nested class; BuildAggregateExecuteExpression comment on MethodInfo reuse
- `Source/LinqToDB/Internal/Linq/Builder/ExpressionBuilder.Associations.cs` -- IsAssociationInRealization third branch extended (PR #5548): guard now also fires when member.DeclaringType != expression.Type, uses expression.Type for GetAttribute lookup
- `Source/LinqToDB/Internal/Linq/Builder/ExpressionBuilder.cs` (lines 1-200) -- no net-new structural changes visible in scanned range; confirmed Build<T> / FindBuilderImpl unchanged
- `Source/LinqToDB/Internal/Linq/Builder/UpdateBuilder.cs` -- NeedsConversion static helper (PR #5506): skips ValueConverter wrap for SqlParameter/SqlValue/SqlColumn/SqlField/SqlAnchor; SqlExpression/SqlFunction with target-column parameter reference also skip
- `Source/LinqToDB/Linq/Expressions.cs` (lines 1-80, public LinqToDB.Linq) -- MapMember overload surface; no aggregation-relevant changes in scanned range (outside area globs)
- `Source/LinqToDB/Linq/Translation/AggregateFunctionBuilder.cs` -- IsServerSideOnly/ItemTransform/ValueTransform on ModeConfig; Build(isExpression) lenient bail-out path via Skipped(); Combine propagates _skipped (PR #5557)
...
- `Source/LinqToDB/Linq/Translation/WindowFunctionsMemberTranslator.cs` (lines 1-120) -- registration surface for window functions (Sum/Avg/Min/Max/RowNumber/Rank/etc.); no aggregate-pipeline changes in scanned range
