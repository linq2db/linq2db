...
- `ProviderName` -- string-constant catalog of every provider/dialect identifier the runtime recognizes (`Source/LinqToDB/ProviderName.cs:23`). As of PR #5451, includes `DuckDB = "DuckDB"` (`Source/LinqToDB/ProviderName.cs:341`). The file-level TODO at `:18` notes that all provider-specific entries are scheduled for obsolescence in v6; `ProviderName` is intended for dialect names only going forward.
- `DataType`, `DbDataType`, `TableOptions`, `SqlJoinType`, `MergeOperationType`, `TakeHints` -- public enums and structs consumed by mapping, SQL emission, and merge/insert APIs. `DbDataType` is a value struct carrying `(SystemType, DataType, DbType, Length, Precision, Scale)` with fluent `WithXxx` copy methods (`Source/LinqToDB/DbDataType.cs:15`). `SqlJoinType` exposes `Inner`/`Left`/`Right`/`Full` (`Source/LinqToDB/SqlJoinType.cs:7`). `TakeHints` is a `[Flags]` enum supporting `Percent` and `WithTies` (`Source/LinqToDB/TakeHints.cs:11`). `DataType` carries an `Array` flag bit (`Array = unchecked((int)0x8000_0000)`) and composite values `Vector32 = Array | Single`, `Vector16 = Array | Half` for vector types; `[Flags]` on the enum is noted with a `// ??? TODO: remove Flags in v7` comment (`Source/LinqToDB/DataType.cs:11`).
...
- `IAsQueryableBuilder<T>`, `IAsQueryableExceptBuilder<T>` -- new public interfaces (PR #5495) in `LinqToDB.Linq` namespace that parameterize the `LinqExtensions.AsQueryable<T>(source, dataContext, configure)` overload. `IAsQueryableBuilder<T>` requires picking `Parameterize()` or `Inline()` as the default rendering mode; `IAsQueryableExceptBuilder<T>` exposes `Except(params Expression<Func<T,object?>>[] members)` to flip the mode for specific members. The entire chain is captured as an expression tree -- calling it outside a query context is undefined behaviour (`Source/LinqToDB/Linq/IAsQueryableBuilder.cs:15`, `Source/LinqToDB/Linq/IAsQueryableExceptBuilder.cs:12`).

## Sql class -- timestamp and date/time functions

`Sql.cs` exposes several server-side date/time properties with distinct client-side fallback semantics (`Source/LinqToDB/Sql/Sql.cs:1166`--`:1198`):

- `Sql.GetDate()` -- emits `CURRENT_TIMESTAMP` (or provider equivalent: `GetDate()` on Sybase/SQL CE, `Now` on Access, `CURRENT` on Informix); client-side returns `DateTime.Now`.
- `Sql.CurrentTimestamp` -- server-side-only property; throws `ServerSideOnlyException` if called outside a query context.
- `Sql.CurrentTimestampUtc` -- evaluates `DateTime.UtcNow` client-side; server-side SQL is provider-dispatched via translator virtuals introduced in PR #5467 (`TranslateServerNow`, `TranslateNow`, `TranslateUtcNow`, `TranslateZonedNow`, `TranslateZonedUtcNow`). Five virtual translation points allow per-provider control over which SQL function is emitted and whether a time-zone conversion is applied.
- `Sql.CurrentTimestamp2` -- dual-mode: same server-side SQL as `CurrentTimestamp`, but evaluable in-process returning `DateTime.Now`.
- `Sql.CurrentTzTimestamp` -- `DateTimeOffset.Now` client-side; server-side emits a provider-specific zoned timestamp expression.

PR #5517 fixed `DateTime.Date` truncation: the cast to `date` now preserves the source column's `DbType`, preventing unintended type coercion when the column carries an explicit provider type.
...
`DataOptionsExtensions` is split across two files. The main file (`DataOptionsExtensions.cs`) covers `LinqOptions`, `SqlOptions`, `ConnectionOptions`, `DataContextOptions`, `BulkCopyOptions`, and miscellaneous helpers (`UseConnectionString`, `UseProvider`, `UseInterceptor`, `RemoveInterceptor`, `UseEnableContextSchemaEdit`, etc.). The provider file (`DataOptionsExtensions.Provider.cs`) contains `Use<Database>` overloads for every supported provider (SQL Server, Oracle, PostgreSQL, MySQL, SQLite, Access, DB2, Firebird, Informix, SAP HANA, SQL CE, SAP/Sybase ASE, ClickHouse, DuckDB), following the overload convention documented in the file's leading comment: two overloads for single-dialect providers, four overloads for multi-dialect/provider databases. Provider overloads always delegate to `<Provider>Tools.ProviderDetector.CreateOptions(options, dialect, provider)` then apply the `optionSetter` callback (`Source/LinqToDB/DataOptionsExtensions.Provider.cs:55`). DuckDB is a single-dialect provider and uses only two overloads; it delegates to `DuckDBTools.GetDataProvider()` directly (no `ProviderDetector`) because there is no dialect or ADO.NET provider variant (`Source/LinqToDB/DataOptionsExtensions.Provider.cs:999`).
...
Read in full (2026-05-11 delta): `AnalyticFunctions.cs` (re-read -- minor touch, no structural changes), `DataOptionsExtensions.Provider.cs` (DuckDB overloads added), `DataType.cs` (no new values since prior read), `ProviderName.cs` (DuckDB constant added), `Sql/Sql.cs` (CurrentTimestamp* properties confirmed), `Sql/Sql.DateTime.cs` (DateTime.Date DbType preservation), `Sql/Sql.DateOnly.cs`, `Sql/Sql.DateTimeOffset.cs`, `Linq/IAsQueryableBuilder.cs` (new), `Linq/IAsQueryableExceptBuilder.cs` (new), `Linq/Expressions.cs` (MapMember overloads, unchanged surface), `CompatibilitySuppressions.xml`, `PublicAPI/PublicAPI.Unshipped.txt`.
...
- **`AsQueryable` expression-tree configuration pattern.** `LinqExtensions.AsQueryable<T>(source, dataContext, configure)` (PR #5495) accepts a `configure` lambda typed as `Expression<Func<IAsQueryableBuilder<T>, IAsQueryableExceptBuilder<T>>>`. The lambda is never invoked directly -- it is captured as an expression tree and interpreted at query-build time by `Methods.LinqToDB.AsQueryableConfigured`. The `IAsQueryableBuilder<T>` / `IAsQueryableExceptBuilder<T>` interfaces are pure marker types; their implementations are internal to the LINQ area.
...
- `DataType` carries `[Flags]` with a `// ??? TODO: remove Flags in v7` note (`Source/LinqToDB/DataType.cs:11`); the `Array` bit-flag and composite `Vector32`/`Vector16` values are the only current consumers of the flags semantics.
- `ProviderName` has a `// TODO: v6: obsolete/remove all provider-specific entries` note (`Source/LinqToDB/ProviderName.cs:18`) -- the class is intended to become dialect-only going forward.
...
- Tier 2 (visited / total): 57 / 58 (~98%) done
  ...
  - Read in full (2026-05-06 coverage-fill): 47 files -- see prior coverage block
  - Read (2026-05-11 delta): Source/LinqToDB/AnalyticFunctions.cs, Source/LinqToDB/DataOptionsExtensions.Provider.cs, Source/LinqToDB/DataType.cs, Source/LinqToDB/ProviderName.cs, Source/LinqToDB/Sql/Sql.cs, Source/LinqToDB/Sql/Sql.DateTime.cs, Source/LinqToDB/Sql/Sql.DateOnly.cs, Source/LinqToDB/Sql/Sql.DateTimeOffset.cs, Source/LinqToDB/Linq/IAsQueryableBuilder.cs, Source/LinqToDB/Linq/IAsQueryableExceptBuilder.cs, Source/LinqToDB/Linq/Expressions.cs, Source/LinqToDB/CompatibilitySuppressions.xml, Source/LinqToDB/PublicAPI/PublicAPI.Unshipped.txt
  - Skipped: 1 -- declaration-only file where type signature is fully captured by the head probe
