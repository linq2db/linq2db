...
- `LinqToDB.Common.Configuration` -- public static class of global flags and option-record proxies. The flat properties (`MaxBinaryParameterLengthLogging`, `MaxStringParameterLengthLogging`, `MaxArrayParameterLengthLogging`, `OptimizeForSequentialAccess`, `TraceMaterializationActivity`, `TranslationThreadMaxHopCount`, `UseNullableTypesMetadata`) are independently mutable; nested `Linq` / `Sql` / `RetryPolicy` / `BulkCopy` / `DataContext` / `Connection` / `QueryTrace` classes wrap the corresponding `*.Default` `OptionsContainer<T>` (`Source/LinqToDB/Common/Configuration.cs:24`, `:147`, `:494`, `:582`, `:697`). Two additional nested classes exist outside the option-record pattern: `Configuration.SqlServer` (`:457`) -- single flag `UseSchemaOnlyToGetSchema` governing SQL Server schema-provider behavior -- and `Configuration.LinqService` (`:471`) -- two flags controlling remote-context type serialization (`SerializeAssemblyQualifiedName`, `ThrowUnresolvedTypeException`). `UseNullableTypesMetadata` (`:98`) uses the C# 14 `field` keyword in its setter to guard against enabling the flag on platforms where nullable metadata is unavailable (`Nullability.EnsureSupport()`).
...
- `System.Data.Linq.Binary` (compatibility shim) -- pre-`net8.0` only; reimplements the `System.Data.Linq.Binary` type for non-NETFX TFMs so the `Converter` static ctor string<->`Binary` registrations link cleanly (`Source/LinqToDB/Compatibility/System/Data/Linq/Binary.cs:1`, `:12`). Gated `#if !NETFRAMEWORK`. `Equals(Binary? other)` and `Equals(object? obj)` carry `[NotNullWhen(true)]` NRT annotations (`:43`, `:57`).
...
- Cross-cutting design invariants the area must preserve: [`code-design.md`](../../../docs/code-design.md) -> "Public API is a contract" (every type listed under "Key types" with `[PublicAPI]` is a stability commitment), "Cross-cutting internals are shared" (do not reshape `MemberAccessor` for a local fix).
...
- Read (this run -- delta):
  - Source/LinqToDB/Common/Configuration.cs -- added `MaxStringParameterLengthLogging` (`:77`) and `MaxArrayParameterLengthLogging` (`:88`) logging-truncation properties; `Configuration.SqlServer` nested class (`:457`) with `UseSchemaOnlyToGetSchema`; `Configuration.LinqService` nested class (`:471`) with `SerializeAssemblyQualifiedName` / `ThrowUnresolvedTypeException`; `UseNullableTypesMetadata` (`:98`) uses C# 14 `field` keyword in setter to guard platform availability via `Nullability.EnsureSupport()`. All existing claims confirmed accurate.
  - Source/LinqToDB/Compatibility/System/Data/Linq/Binary.cs -- `[NotNullWhen(true)]` NRT annotation added to `Equals(Binary? other)` (`:43`) and `Equals(object? obj)` (`:57`). No behavioral change; annotation improves nullable-flow analysis for callers. All existing claims confirmed accurate.
</details>
