...
- **Configuration applier surface.** `ConnectionOptions` is the central record holding everything `DataConnection` needs to bind to a real `DbConnection`: `ConfigurationString`, `ConnectionString`, `IDataProvider`, `ProviderName`, `MappingSchema`, an existing `DbConnection` or `DbTransaction`, `DisposeConnection` flag, `ConnectionFactory` lambda, `DataProviderFactory` lambda, `ConnectionOptionsConnectionInterceptor` (a named wrapper type for the connection interceptor -- not the raw `IConnectionInterceptor` interface), `OnEntityDescriptorCreated` callback. It implements `IApplicable<DataConnection>`/`IApplicable<DataContext>`/`IApplicable<RemoteDataContextBase>` and `IReapplicable<DataConnection>`/`IReapplicable<DataContext>`/`IReapplicable<RemoteDataContextBase>` so the same record drives all three context kinds. `ConfigurationApplier.Apply(DataConnection, ConnectionOptions)` is a 7-tuple switch. `ConfigurationApplier.Reapply` enforces not-changeable-dynamically rules -- only `MappingSchema` and `ConnectionInterceptor` can swap on a live context; everything else throws `LinqToDBException`.
...
- `ConnectionOptions`, `RetryPolicyOptions`, `QueryTraceOptions` -- sealed `record` option types implementing `IOptionSet` + `IApplicable<DataConnection>` + `IReapplicable<...>`. `ConnectionOptions` additionally implements `IApplicable`/`IReapplicable` for `DataContext` and `RemoteDataContextBase`; `QueryTraceOptions` and `RetryPolicyOptions` are scoped to `DataConnection` only.
- `ConnectionOptionsConnectionInterceptor` -- the named wrapper type used for the `ConnectionInterceptor` parameter in `ConnectionOptions` (distinct from the raw `IConnectionInterceptor` interface).
...
- **C# 14 field-backed property setters on `Default`.** `BulkCopyOptions.Default` and `ConnectionOptions.Default` use the C# 14 field-backed property setter syntax. `QueryTraceOptions.Default` and `RetryPolicyOptions.Default` use an explicit backing field (`_default`) instead -- both patterns call `DataConnection.ResetDefaultOptions()` and `ConnectionOptionsByConfigurationString.Clear()` in the setter to invalidate cached configurations.
...
- **Obsolete 20-parameter `BulkCopyOptions` copy ctor retained for binary compatibility.** Marked `[EditorBrowsable(Never)]` and `[Obsolete(...)]`; forwards to the 21-parameter primary ctor passing `default` for `ConflictAction` (`Source/LinqToDB/Data/BulkCopyOptions.cs:169--214`). Will be removed in v7.
...
Read (this run -- delta):
  - Source/LinqToDB/Data/BulkCopyOptions.cs -- Default now uses C# 14 field-backed property setter; obsolete 20-param copy ctor marked [EditorBrowsable(Never)] + [Obsolete(...)]; all 21 params confirmed including ConflictAction
  - Source/LinqToDB/Data/ConnectionOptions.cs -- ConnectionInterceptor typed as ConnectionOptionsConnectionInterceptor? (named wrapper, not raw IConnectionInterceptor); IReapplicable now covers DataContext and RemoteDataContextBase in addition to DataConnection; Default uses C# 14 field-backed setter; internal saved-state fields confirmed
  - Source/LinqToDB/Data/QueryTraceOptions.cs -- 4 params confirmed (TraceLevel, OnTrace, WriteTrace, TraceSwitch); ConfigurationID hashes TraceSwitch.DisplayName + TraceSwitch.Description; IApplicable/IReapplicable only for DataConnection; uses explicit _default backing field
  - Source/LinqToDB/Data/RetryPolicy/RetryPolicyOptions.cs -- Default has non-zero defaults: MaxRetryCount=5, MaxDelay=30s, RandomFactor=1.1, ExponentialBase=2, Coefficient=1s; IApplicable/IReapplicable only for DataConnection; uses explicit _default backing field
</details>
