...
- [#2305 Add AsAsyncEnumerable extension method](https://github.com/linq2db/linq2db/pull/2305) — [merged] @MaceWindu As a follow-up to #2303, here's my proposal to add an `AsAsyncEnumerable()` extension method to `LinqToDB.AsyncExtensions`.  It also changes `ExpressionQuery` so that it implements `IAsyncEnumerable`, so third-party code can directly cast the `IQueryable<T>` to `IAsync...
