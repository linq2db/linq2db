...
- **Truncation to date** — `toDate32(value)` cast.
...
- **`now()` / `now('UTC')`** for `Sql.GetDate()` / `Sql.CurrentTimestampUtc`.
...
- **JSON type** — still experimental in ClickHouse; all three providers have known limitations (Octonica/Driver: don't support it; MySqlConnector: returns non-JSON data). Noted in README.md.
...
- **Engine configuration API** — `BuildEndCreateTableStatement` only produces `MergeTree(ORDER BY ...)` or `Memory()`; no API to configure `ReplacingMergeTree`, `SummingMergeTree`, `Distributed`, projections, etc. The comment says "For now we use fixed engines and it is recommended to use RAW SQL for table creation" (`ClickHouseSqlBuilder.cs:196`).
- **IPv4 via MySqlConnector** — ClickHouse issue #39056 means IPv4 values cannot be read back via the MySQL protocol. Documented in README.md.
...
- `Source/LinqToDB/Internal/DataProvider/ClickHouse/ClickHouseDataProvider.cs` — read in full
- `Source/LinqToDB/Internal/DataProvider/ClickHouse/ClickHouseSqlBuilder.cs` — read in full
- `Source/LinqToDB/Internal/DataProvider/ClickHouse/ClickHouseSqlOptimizer.cs` — read in full
- `Source/LinqToDB/Internal/DataProvider/ClickHouse/ClickHouseProviderAdapter.cs` — read in full
- `Source/LinqToDB/Internal/DataProvider/ClickHouse/ClickHouseProviderDetector.cs` — read in full
- `Source/LinqToDB/Internal/DataProvider/ClickHouse/ClickHouseMappingSchema.cs` — read in full
- `Source/LinqToDB/Internal/DataProvider/ClickHouse/ClickHouseBulkCopy.cs` — read in full
- `Source/LinqToDB/DataProvider/ClickHouse/ClickHouseTools.cs` — read in full
- `Source/LinqToDB/DataProvider/ClickHouse/ClickHouseProvider.cs` — read in full
- `Source/LinqToDB/DataProvider/ClickHouse/ClickHouseOptions.cs` — read in full

### Tier 2 (13/13 read)

- `Source/LinqToDB/Internal/DataProvider/ClickHouse/ClickHouseSqlExpressionConvertVisitor.cs` — read in full
- `Source/LinqToDB/Internal/DataProvider/ClickHouse/Translation/ClickHouseMemberTranslator.cs` — read in full
- `Source/LinqToDB/Internal/DataProvider/ClickHouse/ClickHouseSchemaProvider.cs` — read in full
- `Source/LinqToDB/Internal/DataProvider/ClickHouse/ClickHouseSpecificQueryable.cs` — read in full
- `Source/LinqToDB/Internal/DataProvider/ClickHouse/ClickHouseSpecificTable.cs` — read in full
- `Source/LinqToDB/DataProvider/ClickHouse/ClickHouseHints.cs` — read in full
- `Source/LinqToDB/DataProvider/ClickHouse/ClickHouseHints.generated.cs` — read in full
- `Source/LinqToDB/DataProvider/ClickHouse/ClickHouseRetryPolicy.cs` — read in full
- `Source/LinqToDB/DataProvider/ClickHouse/ClickHouseTransientExceptionDetector.cs` — read in full
- `Source/LinqToDB/DataProvider/ClickHouse/ClickHouseSpecificExtensions.cs` — read in full
- `Source/LinqToDB/DataProvider/ClickHouse/IClickHouseSpecificQueryable.cs` — read in full
- `Source/LinqToDB/DataProvider/ClickHouse/IClickHouseSpecificTable.cs` — read in full
- `Source/LinqToDB/DataProvider/ClickHouse/ClickHouseFactory.cs` — read in full
