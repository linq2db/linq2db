...
`Source/LinqToDB/Internal/DataProvider/Sybase/SybaseDataProvider.cs:139`
...
`Source/LinqToDB/Internal/DataProvider/Sybase/SybaseDataProvider.cs:80`

`SupportedTableOptions` — supports temporary table prefix conventions (`#name` = local, `##name` = global), `CreateIfNotExists`, `DropIfExists`.
`Source/LinqToDB/Internal/DataProvider/Sybase/SybaseDataProvider.cs:98`
...
`Source/LinqToDB/Internal/DataProvider/Sybase/SybaseMappingSchema.cs:28`
...
- `DateFunctionsTranslator` — `DatePart(part, expr)`, `DateAdd(part, inc, expr)`, `GetDate()`, `GETUTCDATE()`. `MakeDateTime` constructs via string concatenation with `RIGHT('0'+CAST(part AS VARCHAR(N)), N)` padding and a final `CAST`. `TranslateDateTimeTruncationToDate` uses `CONVERT(date, expr)`.
...
| DateTime lower bound | 1753-01-01 | `SybaseMappingSchema.cs:33` |
...
| `SybaseMappingSchema` | `Internal/.../SybaseMappingSchema.cs` | Type defaults, literal converters; sub-schemas for native/managed |
...
| `SybaseMemberTranslator` | `Internal/.../Translation/SybaseMemberTranslator.cs` | .NET member → ASE SQL function translation |
