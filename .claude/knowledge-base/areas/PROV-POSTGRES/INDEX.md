...
- Value-to-SQL converters for `bool` (native `TRUE`/`FALSE`), `string`, `char`, `byte[]` (hex-escaped `E'\\x...'::bytea`), `Guid` (`'...'::uuid`), `DateTime` (inline `'...'::timestamp` or `'...'::date`), `BigInteger`.
...
Selected via `CreateMemberTranslator` in `PostgreSQLDataProvider`: `>= v13` → `PostgreSQL13MemberTranslator`, else `PostgreSQLMemberTranslator` (`PostgreSQLDataProvider.cs:101–108`).
...
- `PostgreSQLHints.generated.cs`: T4-generated typed overloads (`ForUpdateHint`, `ForUpdateNoWaitHint`, `ForUpdateSkipLockedHint`, …), one per hint × modifier combination (12 methods).
...
| `DataProvider/.../IPostgreSQLSpecificTable.cs` | Specific table marker |
...
- `Source/LinqToDB/Internal/DataProvider/PostgreSQL/Translation/PostgreSQLMemberTranslator.cs` — member translator
