...
- `string` → `TimeSpan`: parses via `DateTime.Parse(..., DateTimeStyles.NoCurrentDateDefault).TimeOfDay` (`SQLiteMappingSchema.cs:25`).
- `Guid` → SQL: `ConvertGuidToSql` (binary or uppercase TEXT, see above) (`SQLiteMappingSchema.cs:27`).
- `DateTime` → SQL: ISO format `'yyyy-MM-dd HH:mm:ss.fff'` or `'yyyy-MM-dd'` for `DataType.Date` (`SQLiteMappingSchema.cs:68–81`).
- `string`, `char` → SQL: standard string-escape with `char()` for control characters (`SQLiteMappingSchema.cs:96–103`).
- `byte[]` / `Binary` → SQL: hex blob literal `X'...'` (`SQLiteMappingSchema.cs:60–64`).
- Default `string` data type: `NVarChar(255)` (`SQLiteMappingSchema.cs:38`).
...
- **`DateFunctionsTranslator`**: translates `Sql.DateParts` via `strftime()` format codes (`%Y`, `%m`, `%d`, etc.). Quarter is computed as `((month - 1) / 3) + 1`; Millisecond as `CAST(strftime('%f', d) * 1000 AS int) % 1000`. Date arithmetic via `strftime(format, date, 'N unit')` modifiers. `MakeDateTime` constructs via zero-padded string concatenation then `strftime`. UTC timestamp: `DATETIME('now')`.
...
- Tier 1 (visited / total): 10 / 10 ✓
  - Source/LinqToDB/Internal/DataProvider/SQLite/SQLiteDataProvider.cs
  - Source/LinqToDB/Internal/DataProvider/SQLite/SQLiteSqlBuilder.cs
  - Source/LinqToDB/Internal/DataProvider/SQLite/SQLiteSqlOptimizer.cs
  - Source/LinqToDB/Internal/DataProvider/SQLite/SQLiteProviderAdapter.cs
  - Source/LinqToDB/Internal/DataProvider/SQLite/SQLiteProviderDetector.cs
  - Source/LinqToDB/Internal/DataProvider/SQLite/SQLiteMappingSchema.cs
  - Source/LinqToDB/Internal/DataProvider/SQLite/SQLiteBulkCopy.cs
  - Source/LinqToDB/DataProvider/SQLite/SQLiteTools.cs
  - Source/LinqToDB/DataProvider/SQLite/SQLiteProvider.cs
  - Source/LinqToDB/DataProvider/SQLite/SQLiteOptions.cs
- Tier 2 (visited / total): 10 / 10 (100%) ✓
  - Source/LinqToDB/Internal/DataProvider/SQLite/SQLiteSchemaProvider.cs
  - Source/LinqToDB/Internal/DataProvider/SQLite/SQLiteSqlExpressionConvertVisitor.cs
  - Source/LinqToDB/Internal/DataProvider/SQLite/Translation/SQLiteMemberTranslator.cs
  - Source/LinqToDB/Internal/DataProvider/SQLite/SQLiteSpecificTable.cs
  - Source/LinqToDB/DataProvider/SQLite/SQLiteHints.cs
  - Source/LinqToDB/DataProvider/SQLite/SQLiteExtensions.cs
  - Source/LinqToDB/DataProvider/SQLite/SQLiteSpecificExtensions.cs
  - Source/LinqToDB/DataProvider/SQLite/ISQLiteSpecificTable.cs
  - Source/LinqToDB/DataProvider/SQLite/ISQLiteExtensions.cs
  - Source/LinqToDB/DataProvider/SQLite/SQLiteFactory.cs
