...
- String SQL literal: single-quote with `\'` and `\\` escaping (`MySqlMappingSchema.cs:68--73`).
...
- `DateTimeOffset` SQL literal: `CONVERT_TZ('{value:yyyy-MM-dd HH:mm:ss.ffffff}', '{tz}', '+00:00')` at base; `MySql80MappingSchema` overrides to `TIMESTAMP '{value:yyyy-MM-dd HH:mm:ss.ffffffzzz}'` (`MySqlMappingSchema.cs:123--137`).
