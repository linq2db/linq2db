...
`IssueContext` (provider-agnostic, used for non-provider tests): holds `Issue73Entity` (self-referential tree: `Id`, `ParentId?`, `Name`, `Childs`) and `Patent`/`PatentAssessment` (1:1 with `DeleteBehavior.Restrict`). Seeds two `Issue73Entity` rows via `HasData`.
...
| `IssueContextBase` | `Tests/EntityFrameworkCore/Models/IssueModel/IssueContextBase.cs` | `DbContext` for regression tests; holds ~30 `DbSet`s across all tracked issues |
...
### Tier 2 (read this run — 128 of 171)
...
- **PROV-SQLSERVER** — used by `ToolsTests.TestFunctions`, `TestCommandTimeout`, `TestCreateTempTable`, temporal-table tests; SQL Server NorthwindContext; `JsonConvertTests` (AllSqlServer only); SQL Server Fluent maps use `IsTemporal()`.
- **PROV-SQLITE** — used by `SQLiteTests`; default EF provider in many `[EFDataSources]` tests.
- **PROV-POSTGRES** — used by `NpgSqlTests` (range, array, NodaTime); Northwind and IssueModel PostgreSQL context variants; `ForMapping/Npgsql` context.
...
- Tier 2 (visited / total): 128 / 171 (74.9%)
