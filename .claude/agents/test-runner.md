...
1. `Read` `DataProviders.json` (committed; defines connection strings for every standard provider) and `UserDataProviders.json` (if present) at the repo root — `<repoRoot>` when the caller passed one (see *Inputs* → `repoRoot`), otherwise the current working directory.
...
5. **`repoRoot`** — optional absolute path to the repo root the run targets. Default: the current working directory (primary clone). Set it when the change under test lives in a **git worktree** — then `UserDataProviders.json` resolution (pre-condition step 1), the `dotnet test` project path, and the **working directory you run from** all use `<repoRoot>` instead of cwd, so the worktree's code is what gets built and run. **Run `dotnet test` from `<repoRoot>`** (see *Running tests* → the `cd` note): `dotnet` resolves `global.json` — the SDK pin **and** the MTP-vs-VSTest `test.runner` selection — from the current directory, *not* from `--project`, so an inherited cwd whose `global.json` differs from the worktree's (e.g. the primary clone on a branch that hasn't merged a master `global.json` change) makes the run fall back to VSTest and fail on .NET 10 with `Testing with VSTest target is no longer supported by Microsoft.Testing.Platform`.
...
- **The absolute `--project` path alone usually suffices — don't self-block.** When the worktree was branched from the same `origin/master` as the primary clone (the normal case), both share an identical `global.json`, so `dotnet test --project <repoRoot>/<project> …` from the inherited cwd resolves the right SDK/MTP runner with no `cd`. The Bash tool's cwd *does* persist across calls — never abort claiming it doesn't. The `cd <repoRoot>` / caller-PowerShell `Set-Location` step below is the fallback only when the two clones' `global.json` actually diverge.
- **`cd` into `<repoRoot>` first.** When `repoRoot` is set, issue a standalone `cd <repoRoot>` Bash call **before** the `dotnet test` call (the Bash tool's working directory persists across calls within an invocation) — don't rely on the absolute `--project` path alone. `dotnet` reads `global.json` (SDK version + `test.runner`) from the working directory, so running from the inherited primary-clone cwd uses *that* clone's `global.json`; if it lacks `"test": { "runner": "Microsoft.Testing.Platform" }` the run fails with `Testing with VSTest target is no longer supported by Microsoft.Testing.Platform on .NET 10 SDK`. Single `cd` calls are allowed (no chaining); the persisted cwd makes the subsequent `dotnet test` resolve the worktree's `global.json` — needed only when that file diverges from the primary clone's (see the bullet above). **If the `cd`-then-`dotnet test` approach still resolves the wrong `global.json`** (the Bash cwd not persisting as expected — the worktree run then fails with the VSTest-no-longer-supported / `MSB1001: Unknown switch '--project'` error), this agent (Bash-only) can't recover. Abort with a `blocked` status saying the worktree run needs the **caller** to invoke `dotnet test` via the PowerShell tool with `Set-Location <repoRoot>` (which reliably anchors `global.json` resolution and which this agent lacks). Surfaced on PR #5627: this agent blocked on the worktree MTP resolution; the main agent ran it via PowerShell `Set-Location <worktree>; dotnet test --project <abs> -f net10.0 --settings <abs>/.runsettings` successfully.
