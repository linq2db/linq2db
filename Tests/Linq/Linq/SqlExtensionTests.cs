		...
		class CompositeKeyEqualsExtensionBuilder : Sql.IExtensionCallBuilder
		{
			public void Build(Sql.ISqExtensionBuilder builder)
			{
				...
				if (value!.GetType().IsArray)
				{
					...
					builder.ResultExpression = new SqlSearchCondition(new SqlCondition(false, new SqlPredicate.InList(left, null, false, values)));
				}
				else
				{
					var right = new SqlRow(
						keyType.GetRuntimeProperties()
							.Where(prop => !prop.HasAttribute<NotColumnAttribute>())
							.Select(prop => new SqlValue(prop.PropertyType, prop.GetValue(value)))
							.Cast<ISqlExpression>()
							.ToArray());
					builder.ResultExpression = new SqlSearchCondition(new SqlCondition(false, new SqlPredicate.ExprExpr(left, SqlPredicate.Operator.Equal, right, null)));
				}
			}
		}
