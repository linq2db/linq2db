...
namespace Tests.Linq
{
	...
	{
		// MIN takes only numeric arguments, so a boolean can reach this function only through the window clause.
		...
		public void MinWithBooleanWindow([SupportsAnalyticFunctionsContext] string context)
		{
			...
			var query =
				...
				{
					Id           = t.Id,
					ByOrder      = Sql.Window.Min(t.IntValue, w => w.PartitionBy(t.CategoryId).OrderBy(t.IntValue == 20).ThenBy(t.Id)),
					ByPartition  = Sql.Window.Min(t.IntValue, w => w.PartitionBy(t.IntValue == 20).OrderBy(t.Id)),
					ByNullCheck  = Sql.Window.Min(t.IntValue, w => w.PartitionBy(t.NullableIntValue != null).OrderBy(t.Id)),
				};
