...
namespace Tests.Linq
{
	public class CteTests : TestBase
	{
		...
		[ActiveIssue(Configuration = TestProvName.AllYdb, Details = "YDB does not preserve a CTE's inner ORDER BY in the outer SELECT (the ORDER BY there only bounds LIMIT). Proper fix is to propagate the CTE's ORDER BY into the referencing query.")]
		[Test]
		...
		public void Issue3360_TypedNullEnumInAnchor([IncludeDataSources(true, TestProvName.AllSqlServer)] string context)
		{
			...
			var query =
				from node in db.GetCte<Issue3360_TypedNullEnumInAnchorProjection>(
					cte =>
					{
		...
		public void Issue3360_TypedNullEnumInAnchor2([IncludeDataSources(false, TestProvName.AllSqlServer)] string context)
		{
			...
			var query =
				from node in db.GetCte<Issue3360_TypedNullEnumInAnchorProjection>(
					cte => db.GetTable<Issue3360NullInAnchor>()
						.Select(p => new Issue3360_TypedNullEnumInAnchorProjection(p.Id, VarChar<StrEnum?>(null, 50)))
						.Concat(
							from p in db.GetTable<Issue3360NullInAnchor>()
		...
		public void Issue2451_ComplexColumn_All([RecursiveCteContextSource(ProviderName.DB2)] string context)
		{
			...
			var query = db
				.GetCte<Person>(cte =>
					db.Person
						.Select(p => new Person() { FirstName = p.FirstName })
						.Concat(
							from p in cte
							join r in db.Person on p.FirstName equals r.LastName
							select new Person() { FirstName = r.FirstName + '/' + r.LastName }
