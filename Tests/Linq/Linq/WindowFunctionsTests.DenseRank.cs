...
namespace Tests.Linq
{
	partial class WindowFunctionsTests
	{
		...
		public void DenseRankWithNulls([SupportsAnalyticFunctionsContext] string context)
		{
			...
			foreach (var (id, expectedRn7, expectedRn8) in expected)
			{
				byId[id].rn7.ShouldBe(expectedRn7, $"rn7 (ASC NULLS FIRST) mismatch for Id={id}");
				byId[id].rn8.ShouldBe(expectedRn8, $"rn8 (DESC NULLS LAST) mismatch for Id={id}");
			}
		}
