...
namespace Tests.Linq
{
	partial class WindowFunctionsTests
	{
		...
		public void DenseRankWithMultiplePartitions([DataSources(TestProvName.AllOracleNative)] string context)
		{
		...
		[Test]
		[ThrowsForProvider(typeof(LinqToDBException), TestProvName.AllMySql57, ErrorMessage = ErrorHelper.Error_WindowFunction_NotSupported)]
		public void DenseRankWithMultiplePartitionsWithDefineWindow([DataSources(TestProvName.AllOracleNative)] string context)
		{
			using var db    = GetDataContext(context);
			using var table = db.CreateLocalTable(WindowFunctionTestEntity.Seed());
		...
		[Test]
		[ThrowsForProvider(typeof(LinqToDBException), TestProvName.AllMySql57, ErrorMessage = ErrorHelper.Error_WindowFunction_NotSupported)]
		public void DenseRankWithoutPartition([DataSources(TestProvName.AllOracleNative)] string context)
		{
			using var db    = GetDataContext(context);
			using var table = db.CreateLocalTable(WindowFunctionTestEntity.Seed());
