...
namespace Tests.Linq
{
	partial class WindowFunctionsTests
	{
		...
		[ThrowsForProvider(typeof(LinqToDBException),
			TestProvName.AllSQLite, TestProvName.AllSqlServer2008Minus, TestProvName.AllSqlServer2012Plus, TestProvName.AllClickHouse,
		...
		public void VarianceBasic([DataSources] string context)
		{
			...
			var query =
				...
				{
					Id    = t.Id,
					Value = Sql.Window.Variance(t.IntValue, w => w.PartitionBy(t.CategoryId).OrderBy(t.Id)),
				};

			var sql = query.ToSqlQuery().Sql;
			sql.ShouldContain("VARIANCE");
		...
		[ThrowsForProvider(typeof(LinqToDBException),
			...
			ProviderName.Firebird3, ProviderName.Firebird4, TestProvName.AllFirebird5Plus, TestProvName.AllSapHana, TestProvName.AllInformix, ProviderName.Ydb,
			ErrorMessage = ErrorHelper.Error_WindowFunction_Variance)]
		public void VarianceViaWindow([DataSources] string context)
		{
			var data = WindowFunctionTestEntity.Seed();
			...
			var query =
				...
				{
					Id    = t.Id,
					Value = Sql.Window.Variance(t.IntValue, w => w.UseWindow(wnd)),
				};
