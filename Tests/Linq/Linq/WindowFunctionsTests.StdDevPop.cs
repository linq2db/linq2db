	...
	partial class WindowFunctionsTests
	{
		...
		public void StdDevPopBasic([SupportsAnalyticFunctionsContext] string context)
		{
			...
			var query =
				...
				{
					Id    = t.Id,
					Value = Sql.Window.StdDevPop(t.IntValue, w => w.PartitionBy(t.CategoryId).OrderBy(t.Id)),
				};
			...
			foreach (var row in query.ToList())
				AssertRunningStat(row.Value, ExpectedRunningVariance(data, row.Id, population: true), stdDev: true);
		}
		...
		public void StdDevPopViaWindow([SupportsAnalyticFunctionsContext] string context)
		{
			...
			var query =
				...
				{
					Id    = t.Id,
					Value = Sql.Window.StdDevPop(t.IntValue, w => w.UseWindow(wnd)),
				};
