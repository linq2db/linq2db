...
namespace Tests.Infrastructure
{
	...
	public class ParallelExecutionTests
	{
		...
		sealed class CreateDatabaseProbeAttribute : CreateDatabaseSourcesAttribute
		{
			public IEnumerable<string> Resolve() => GetProviders();
		}

		sealed class NorthwindProbeAttribute : NorthwindDataContextAttribute
		{
			public IEnumerable<string> Resolve() => GetProviders();
		}

		sealed class IncludeProbeAttribute : IncludeDataSourcesAttribute
		{
			public IncludeProbeAttribute(params string[] providers) : base(providers)
			{
			}
		...
		/// narrows <see cref="TestConfiguration.UserProviders"/> through <c>TestConfiguration.Providers</c>
		...
		{
			var signalled = new CreateDatabaseProbeAttribute().Resolve().ToHashSet(StringComparer.OrdinalIgnoreCase);

			var laneKeys = new NorthwindProbeAttribute().Resolve()
				.Concat(new IncludeProbeAttribute(TestProvName.NoopProvider).Resolve());
