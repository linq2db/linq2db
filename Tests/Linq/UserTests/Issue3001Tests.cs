...
namespace Tests.UserTests
{
	...
	public class Issue3001Tests : TestBase
	{
		...
		public class Pet
		{
			...
			private static Expression<Func<Pet, House>> HouseExpression()
			{
				return entity => entity.Person.House;
			}

			private static Expression<Func<Pet, bool>> IsHouseMultiLevelExpression()
			{
			...
			string context)
		{
			...
			using var houseTable  = db.CreateLocalTable(new[] {new House  {Id = 1, Levels   = 2}});
			using var personTable = db.CreateLocalTable(new[] {new Person {Id = 1, HouseId  = 1}});
			using var petTable    = db.CreateLocalTable(new[] {new Pet    {Id = 1, PersonId = 1}});
