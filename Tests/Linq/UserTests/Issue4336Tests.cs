...
namespace Tests.UserTests
{
	...
	{
		[Test]
		public void Issue4336Test([DataSources] string context)
		{
		...
		public IQueryable<ViewProductCategoryCapacityRow> ViewProductCategoryCapacity(IDataContext db) =>
		...
		public IQueryable<ViewProductCapacityRow> ViewProductCapacity(IDataContext db) =>
		...
		public IQueryable<ViewSumOfOrderedProductsByPeriodRow> ViewSumOfOrderedProductsByPeriod(IDataContext db) =>
		...
		public IQueryable<ViewCapacityAndOrderedByPeriodAndProductRow> ViewCapacityAndOrderedByPeriodAndProduct(IDataContext db) =>
						  from op in db.GetTable<OrderPeriod>()
		...
		public IQueryable<ViewSumOfOrderedCategoriesByPeriodRow> ViewSumOfOrderedCategoriesByPeriod(IDataContext db) =>
		   from op in db.GetTable<OrderPeriod>()
		   from oh in op.OrderHeaders.DefaultIfEmpty()
		   from oi in oh.OrderItems.DefaultIfEmpty()
		...
		public IQueryable<ViewCapacityAndOrderedByPeriodAndProductCategoryRow> ViewCapacityAndOrderedByPeriodAndProductCategory(IDataContext db) =>
		   from op in db.GetTable<OrderPeriod>()
		...
		public IQueryable<ViewCapacityAndOrderedByPeriodRow> ViewCapacityAndOrderedByPeriod(IDataContext db) =>
		...
		public static T Coalesce<T>([ExprParameter] T expr, [ExprParameter] T nullValue)
		{
			throw new LinqException($"'{nameof(Coalesce)}' is server-side method.");
		}
	}
}
