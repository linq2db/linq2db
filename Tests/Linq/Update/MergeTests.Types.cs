...
namespace Tests.Merge
{
	public partial class MergeTests
	{
		...
		private static readonly MergeTypes[] InitialTypes1Data = new[]
		{
			...
			{
				Id = 2,
				FieldInt32 = int.MinValue,
				FieldInt64 = long.MinValue,
				FieldBoolean = true,
			...
			new MergeTypes()
			{
				...
				FieldDateTime = new DateTime(2098, 10, 12, 21, 14, 15, 999),
		...
		private static readonly MergeTypes[] InitialTypes2Data = new[]
		{
			new MergeTypes()
			{
				...
				FieldFloat = -float.Epsilon,
				FieldDouble = double.Epsilon,
				FieldDateTime = new DateTime(2098, 10, 12, 21, 14, 15, 909),
		...
		public void TestMergeTypes(string context)
		{
			using (var db = new TestDataConnection(context))
			{
				...
				{
					AssertTypesRow(InitialTypes1Data[i], result1[i]);
				}

				for (var i = 0; i < InitialTypes2Data.Length; i++)
				{
					AssertTypesRow(InitialTypes2Data[i], result2[i]);
				}
			}
		}

		private void AssertTypesRow(MergeTypes expected, MergeTypes actual)
		{
			...
			Assert.AreEqual(expected.FieldTime, actual.FieldTime);
