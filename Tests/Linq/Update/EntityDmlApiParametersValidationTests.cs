...
{
	/// <summary>
	/// Null-argument validation for the entity-builder <c>Insert</c> / <c>InsertAsync</c> and
	/// <c>Update</c> / <c>UpdateAsync</c> overloads — mirrors the <c>UpsertTests</c> /
	/// <c>MergeTests</c> ApiParametersValidation suites.
	/// </summary>
	[TestFixture]
	public class EntityDmlApiParametersValidationTests
	{
		[Table("EntityDmlApiValidation")]
		...
		public static IEnumerable<TestCaseData> NullParameterCases
		{
			get
			{
				var table = new FakeTable<Row>();
				var item  = new Row();

				Expression<Func<IEntityInsertSpec<Row>, IEntityInsertSpec<Row>>> insCfg     = b => b;
				Expression<Func<IEntityInsertSpec<Row>, IEntityInsertSpec<Row>>> insCfgNull = null!;
				Expression<Func<IEntityUpdateSpec<Row>, IEntityUpdateSpec<Row>>> updCfg     = b => b;
				Expression<Func<IEntityUpdateSpec<Row>, IEntityUpdateSpec<Row>>> updCfgNull = null!;

				var cases = new Action[]
				{
					// ------------ entity Insert ------------
					() => LinqExtensions.Insert<Row>(null!, item, insCfg),
					() => LinqExtensions.Insert<Row>(table, null!, insCfg),
					() => LinqExtensions.Insert<Row>(table, item, insCfgNull),

					() => { _ = LinqExtensions.InsertAsync<Row>(null!, item, insCfg); },
					() => { _ = LinqExtensions.InsertAsync<Row>(table, null!, insCfg); },
					() => { _ = LinqExtensions.InsertAsync<Row>(table, item, insCfgNull); },

					// ------------ entity Update ------------
					() => LinqExtensions.Update<Row>(null!, item, updCfg),
					() => LinqExtensions.Update<Row>(table, null!, updCfg),
					() => LinqExtensions.Update<Row>(table, item, updCfgNull),

					() => { _ = LinqExtensions.UpdateAsync<Row>(null!, item, updCfg); },
					() => { _ = LinqExtensions.UpdateAsync<Row>(table, null!, updCfg); },
					() => { _ = LinqExtensions.UpdateAsync<Row>(table, item, updCfgNull); },
				};

				return cases.Select((d, i) => new TestCaseData(d).SetName($"EntityDml.API.NullParameter.{i:D2}"));
			}
		}

		[TestCaseSource(nameof(NullParameterCases))]
		public void EntityDmlApiNullParameter(Action action)
		{
			Action act = () => action();
