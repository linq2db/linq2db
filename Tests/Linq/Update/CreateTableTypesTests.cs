	...
	public class CreateTableTypesTests : TestBase
	{
		...
		static IEnumerable<TestCreateTableColumnTypeParameters> TestCases
		{
			...
			get
			{
				...
				yield return new TestCreateTableColumnTypeParameters("Double"                         , e => e.HasColumn(_ => _.Double),                                 v => v.Double             = 3.14                                , null,                                                                                                                        ctx => ctx.Contains(ProviderName.Firebird25) || ctx.Contains(ProviderName.Firebird3), null);
				// Firebird looses precision of double
