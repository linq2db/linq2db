	...
	public class CreateTableTypesTests : TestBase
	{
		...
		static IEnumerable<TestCreateTableColumnTypeParameters> TestCases
		{
			...
			get
			{
				...
				yield return new TestCreateTableColumnTypeParameters("BooleanNullable"                , e => e.HasColumn(_ => _.BooleanNullable),                        v => v.BooleanNullable    = true                                , (ctx, v) => { if (ctx.Contains("Access")) { v.BooleanNullable = false; } },                                                  null,                            ctx => ctx.IsAnyOf(TestProvName.AllSybase));
				...
				// Oracle treats empty string as null in this context
				// Sybase roundtrips empty string to " " (WAT?)
				yield return new TestCreateTableColumnTypeParameters("String10"                       , e => e.Property (_ => _.String).IsNullable(false).HasLength(10), v => v.String             = "test 10"                           , (ctx, v) => { if (ctx.Contains("Oracle") || ctx.Contains("Sybase")) { v.String = " "; } else { v.String = string.Empty; } }, ctx => ctx.IsAnyOf(TestProvName.AllOracleNative), null);
