  ...
  public class DB2iSeriesTests : DataProviderTestBase {
    ...
    public void TestDataTypes(string context) {
      using (var conn = new DataConnection(context)) {
        ...
        Assert.That(TestType<iDB2Integer?>(conn, "intDataType", DataType.Int32), Is.EqualTo(new iDB2Integer(7777777)));
        ...
        Assert.That(TestType<iDB2Real?>(conn, "realDataType", DataType.Single), Is.EqualTo(new iDB2Real(20.31f)));
        ...
        Assert.That(TestType<iDB2VarChar?>(conn, "charDataType", DataType.Char), Is.EqualTo(new iDB2VarChar("1")));
        ...
        Assert.That(TestType<iDB2Time?>(conn, "timeDataType", DataType.Time), Is.EqualTo(new iDB2Time(new DateTime(0, 0, 0, 12, 12, 12))));
        Assert.That(TestType<DateTime?>(conn, "timestampDataType", DataType.DateTime2), Is.EqualTo(new DateTime(2012, 12, 12, 12, 12, 12, 12)));
        Assert.That(TestType<iDB2TimeStamp?>(conn, "timestampDataType", DataType.DateTime2), Is.EqualTo(new iDB2TimeStamp(new DateTime(2012, 12, 12, 12, 12, 12, 12))));
        ...
        Assert.That(conn.Execute<iDB2Rowid>("SELECT rowid FROM AllTypes WHERE ID = 2").Value.Length, Is.Not.EqualTo(0));
        ...
        Assert.That(TestType<iDB2Decimal?>(conn, "decimalDataType", DataType.Decimal).ToString(), Is.EqualTo(new iDB2Decimal(9999999m).ToString()));
        Assert.That(TestType<iDB2Binary>(conn, "varbinaryDataType", DataType.VarBinary).ToString(), Is.EqualTo(new iDB2Binary(new byte[] { 49, 50, 51, 52 }).ToString()));
        Assert.That(TestType<iDB2DecFloat16?>(conn, "decfloatDataType", DataType.Decimal).ToString(), Is.EqualTo(new iDB2DecFloat16(8888888m).ToString()));
        Assert.That(TestType<iDB2DecFloat34?>(conn, "decfloatDataType", DataType.Decimal).ToString(), Is.EqualTo(new iDB2DecFloat34(8888888m).ToString()));
      }
    }
    ...
    public class ALLTYPE {
      ...
      [Column, Nullable]
      public string VARBINARYDATATYPE { get; set; } // VARCHAR(5)
      [Column, Nullable]
    ...
    public void TestTypes(string context) {
      ...
      var timeValue = DB2iSeriesTypes.Time.CreateInstance(new DateTime(0, 0, 0, 1, 1, 1));
      ...
      Assert.That(DB2iSeriesTypes.DecFloat16.CreateInstance().IsNull, Is.False);
      Assert.That(DB2iSeriesTypes.DecFloat34.CreateInstance().IsNull, Is.False);
