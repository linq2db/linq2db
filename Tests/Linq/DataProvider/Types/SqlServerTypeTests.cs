	...
	public sealed  class SqlServerTypeTests : TypeTestsBase
	{
		...
		public async ValueTask TestVectorType([IncludeDataSources(TestProvName.AllSqlServer2025Plus)] string context)
		{
			...
			if (msClient)
			{
				...
				await TestType<SqlVector<float>, SqlVector<float>?>(context, type, sqlVector3, sqlVector2, filterByValue: false, filterByNullableValue: false, isExpectedValue: v => Enumerable.SequenceEqual(v.Memory.ToArray(), sqlVector3.Memory.ToArray()), isExpectedNullableValue: v => v != null && Enumerable.SequenceEqual(v.Memory.ToArray(), sqlVector2.Memory.ToArray()));
			}
		}
	}
}
