				...
				yield return new ProcedureTestCase(new ProcedureSchema()
				{
					...
					ResultTable         = new TableSchema
					{
						...
						Columns           = new List<ColumnSchema>
						{
							...
							new ColumnSchema { ColumnName = "cidrDataType",        ColumnType = "cidr",                            MemberName = "cidrDataType",        MemberType = "IPNetwork?", SystemType = typeof(IPNetwork), ProviderSpecificType = "NpgsqlInet",   IsNullable = true, DataType = DataType.Udt            },
#else
							new ColumnSchema { ColumnName = "cidrDataType",        ColumnType = "cidr",                            MemberName = "cidrDataType",        MemberType = "ValueTuple<IPAddress, byte>?", SystemType = typeof(ValueTuple<IPAddress, byte>), ProviderSpecificType = "NpgsqlInet",   IsNullable = true, DataType = DataType.Udt            },
#endif
		...
		public void TestMaterializedViewSchema([IncludeDataSources(TestProvName.AllPostgreSQL)] string context)
		{
			using (var db = GetDataConnection(context))
			{
				...
				using (Assert.EnterMultipleScope())
				{
					...
					Assert.That(view.Columns[0].Scale, Is.Zero);
