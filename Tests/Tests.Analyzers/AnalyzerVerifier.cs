...
{
	// Thin wrapper over the Roslyn testing SDK: a snippet is compiled against TestReferenceAssemblies.Default plus -
	// unless one of the VerifyWithoutLinqToDBAsync overloads is used - the real linq2db assembly, so Sql.Ext /
	// Sql.Window symbols resolve. That pack matches the test project's TFM so the loaded linq2db build fits it
	// (a lower ref pack would trip CS1705), which is also why a caller-chosen reference set comes only in the
	// overload that drops linq2db: a lower pack cannot carry that assembly.
		...
		where TAnalyzer : DiagnosticAnalyzer, new()
	{
		...
		{
			return RunAsync(source, withLinqToDB: true, TestReferenceAssemblies.Default, editorConfig, expected);
		}
		...
		{
			return RunAsync(source, withLinqToDB: false, TestReferenceAssemblies.Default, editorConfig: null, expected);
		}
