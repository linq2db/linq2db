...
namespace Tests.Analyzers
{
	...
	public sealed class ServerSideOnlyContractAnalyzerTests
	{
		...
		// The one input where the explicit argument and the Sql.Extension ctor default disagree, so the only one
		// that pins their order. The Sql.Function cases above cannot: FunctionAttribute does not derive from
		// ExtensionAttribute, so unset already yields false there. Reading the fallback first would call this
		// MARKED and report L2DB1004, after which the fix rewrites the throw and leaves a still-client-evaluable
		// member claiming to be server-side API. NotImplementedException keeps arm B out of it, so a regression
		// changes the reported id rather than only the fix target.
		[Test]
		public Task ReportsExtensionStubWithServerSideOnlyExplicitlyFalse()
		{
			var source = Usings + """
				static class C
				{
					[{|#1:Sql.Extension("F", ServerSideOnly = false)|}]
					public static int {|#0:M|}() => throw new NotImplementedException();
				}
				""";
		...
		// GeneratedCodeAnalysisFlags.None on the shipped analyzer is what makes a scaffolded model silent - a
		// consumer cannot edit generated code, so a diagnostic there is noise they can only suppress. It is also
		// the deliberate opposite of the internal host's value, which is the kind of asymmetry a later reader
		// harmonises away. Roslyn keys on the <auto-generated> header, not on the file name.
		[Test]
		public Task DoesNotReportInGeneratedCode()
		{
		...
		// Marker form 2 by ctor default with a wrong exception - the Issue773Tests.cs:34 / FunctionTests.cs:767
		// shape the corpus fix rewrote. This form's ServerSideOnlyException negative is already pinned by
		// DoesNotReportBareSqlExtensionStub, so this slot covers the positive instead.
		[Test]
		public Task ReportsBareSqlExtensionStubThrowingWrongException()
		{
			var source = Usings + """
				static class C
				{
