...
{
	public class ForMappingTests : TestBase
	{
		private static HashSet<string> _createdDbs = new HashSet<string>();

		private static DbContextOptionsBuilder<TContext> InitializeContext<TContext>(Func<DataOptions, DataOptions>? optionsSetter = null)
			where TContext : ForMappingContextBase
		{
		...
		private ForMappingContextBase CreateContext(string provider, Func<DataOptions, DataOptions>? optionsSetter = null)
		{
			ForMappingContextBase ctx;
			var connectionString = DataConnection.GetConnectionString(provider);
			if (provider.IsAnyOf(TestProvName.AllPostgreSQL))
			{
				var options = InitializeContext<PostgreSQL.Models.ForMapping.ForMappingContext>()
					.UseNpgsql(connectionString)
					.Options;

				ctx = new PostgreSQL.Models.ForMapping.ForMappingContext(options);
			}
			else if (provider.IsAnyOf(TestProvName.AllMySql))
			{
				var options = InitializeContext<Pomelo.Models.ForMapping.ForMappingContext>()
#if !NETFRAMEWORK
				...
				ctx = new Pomelo.Models.ForMapping.ForMappingContext(options);
			}
			else if (provider.IsAnyOf(TestProvName.AllSQLite))
			{
				var options = InitializeContext<SQLite.Models.ForMapping.ForMappingContext>()
					.UseSqlite(connectionString)
					.Options;

				ctx = new SQLite.Models.ForMapping.ForMappingContext(options);
			}
			else if (provider.IsAnyOf(TestProvName.AllSqlServer))
			{
				var options = InitializeContext<SqlServer.Models.ForMapping.ForMappingContext>()
					.UseSqlServer(connectionString)
					.Options;

				ctx = new SqlServer.Models.ForMapping.ForMappingContext(options);
			}
			else
				throw new InvalidOperationException($"Context initialization missing for provider {provider}");

			if (_createdDbs.Add(connectionString))
			{
