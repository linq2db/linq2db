	...
	public class SqlQuery : ISqlTableSource
	{
		...
		public abstract class Predicate : ISqlPredicate
		{
			...
			public class ExprExpr : Expr
			{
				public ExprExpr(ISqlExpression exp1, Operator op, ISqlExpression exp2)
					: base(exp1, LinqToDB.Sql.Precedence.Comparison)
				{
					this.Operator = op;
					Expr2 = exp2;
				}
			...
			public class Like : NotExpr
			{
				public Like(ISqlExpression exp1, bool isNot, ISqlExpression exp2, ISqlExpression escape)
					: base(exp1, isNot, LinqToDB.Sql.Precedence.Comparison)
				{
					Expr2  = exp2;
					Escape = escape;
				}
		...
		public class Condition : IQueryElement, ICloneableElement
		{
			...
			public int Precedence
			{
				...
				{
					return
						IsNot ? LinqToDB.Sql.Precedence.LogicalNegation :
						IsOr  ? LinqToDB.Sql.Precedence.LogicalDisjunction :
						        LinqToDB.Sql.Precedence.LogicalConjunction;
				}
			}
		...
		public class SearchCondition : ConditionBase<SearchCondition, SearchCondition.Next>, ISqlPredicate, ISqlExpression
		{
			...
			public int Precedence
			{
				get
				{
					if (_conditions.Count == 0) return LinqToDB.Sql.Precedence.Unknown;
					if (_conditions.Count == 1) return _conditions[0].Precedence;

					return _conditions.Select(_ =>
						_.IsNot ? LinqToDB.Sql.Precedence.LogicalNegation :
						_.IsOr  ? LinqToDB.Sql.Precedence.LogicalDisjunction :
						          LinqToDB.Sql.Precedence.LogicalConjunction).Min();
				}
			}
		...
		static void ConcatSearchCondition(WhereClause where1, WhereClause where2)
		{
			...
			{
				if (where1.SearchCondition.Precedence < LinqToDB.Sql.Precedence.LogicalConjunction)
				{
				...
				if (where2.SearchCondition.Precedence < LinqToDB.Sql.Precedence.LogicalConjunction)
				{
						...
						case QueryElementType.ExprExprPredicate :
							{
								...
								if (ee.Operator == Predicate.Operator.Equal || ee.Operator == Predicate.Operator.NotEqual)
								{
									...
									return new Predicate.Expr(new SqlValue(value), LinqToDB.Sql.Precedence.Comparison);
								}
							}
			...
			if (p.Values.Count == 1 && p.Values[0] is SqlParameter)
			{
				...
				if (pr.Value is IEnumerable && p.Expr1 is ISqlTableSource)
				{
...

					{
						...
						if (p.IsNot)
							return new Predicate.NotExpr(sc, true, LinqToDB.Sql.Precedence.LogicalNegation);

						return new Predicate.Expr(sc, LinqToDB.Sql.Precedence.LogicalDisjunction);
					}
				}
		...
		public int Precedence
		{
			get { return LinqToDB.Sql.Precedence.Unknown; }
		}
