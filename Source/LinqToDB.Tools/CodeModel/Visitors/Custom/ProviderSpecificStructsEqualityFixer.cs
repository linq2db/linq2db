	...
	public sealed class ProviderSpecificStructsEqualityFixer : ConvertCodeModelVisitor
	{
		...
		protected override ICodeElement Visit(CodeBinary expression)
		{
				...
				&& _types.Contains(expression.Right.Type))
			{
				...
				// x == y => x.Equals(y)
				// x != y => !x.Equals(y)
				ICodeExpression result = new CodeCallExpression(false, expression.Left, WellKnownTypes.System.Object_Equals, Array<IType>.Empty, true, new []{ expression.Right }, WellKnownTypes.System.Boolean);
