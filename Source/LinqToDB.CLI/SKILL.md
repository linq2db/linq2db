...
- Output is streamed while rows are read, so stdout can contain a partial result when the command is cancelled or fails. An `--output-file` target is written through a temporary file in the same directory and moved into place only after the command completes successfully, so it is never left with partial output. A hidden temporary file can remain if the process is killed.
