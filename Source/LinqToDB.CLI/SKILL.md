...
- **Windows with UTF-8 codepage:** if DB2 or Informix connections fail with `ERROR - no error information available` (no `SQLSTATE`, nothing written to any log), set the environment variable `DB2CODEPAGE=1208`. When Windows is configured with *Use Unicode UTF-8 for worldwide language support*, the ANSI and OEM codepages are both `65001`, and IBM's native client cannot derive its codepage from that — so it fails to allocate a CLI environment handle before any connection is attempted. This affects both DB2 and Informix, because both use `IBM.Data.Db2`. To confirm the diagnosis independently of linq2db, run `db2cli.exe validate -connect -connstring "<your connection string>"` from the IBM driver's `clidriver\bin`: `Failed to alloc env handle` is this problem.
...
- Output is streamed while rows are read, so stdout can contain a partial result when the command is cancelled or fails. An `--output-file` target is written through a temporary file in the same directory and moved into place only after the command completes successfully, so it is never left with partial output. A leftover temporary file can remain if the process is killed.
