...
- Provider-specific output formatting has special handling validated for SQL Server and Oracle provider-specific types. The command still uses `DbDataReader.GetProviderSpecificValue` for all providers and falls back to invariant string conversion, but additional provider-specific formatting rules should be added incrementally as provider behavior is tested.
