...
An optional top-level `mcp` section can set instance-specific `title`, `description`, and `instructions` returned during MCP initialization. It can also set `maxResponseBytes`, the server-wide UTF-8 size limit for primary buffered query and execute output content; protocol framing and the short truncation warning are not counted. Use it to distinguish servers registered for different application or database domains. The `mcp` section is not a connection profile; `config-init` preserves it but does not create or modify it.
