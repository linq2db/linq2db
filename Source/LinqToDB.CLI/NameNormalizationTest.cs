...
namespace LinqToDB.Tools
{
	...
	internal static class NameNormalizationTest
	{
		...
		public static void NormalizationTest()
		{
			...
			CreateClass(builder, classes.New(builder.Name(DUPLICATE_NAME1)), false, false, DUPLICATE_NAME1, DUPLICATE_NAME2, true, true);
			CreateClass(builder, classes.New(builder.Name(DUPLICATE_NAME2)), false, false, DUPLICATE_NAME2, DUPLICATE_NAME1, true, false);

			CreateClass(builder, classes.New(builder.Name(DUPLICATE_NAME1)), false, false, DUPLICATE_NAME1, DUPLICATE_NAME2, false, true);
			CreateClass(builder, classes.New(builder.Name(DUPLICATE_NAME2)), false, false, DUPLICATE_NAME2, DUPLICATE_NAME1, false, false);
			...
			File.WriteAllText(@"..\..\..\Generated\Test.cs", sourceCode[0].Code);
		}
		...
		private static CodeClass CreateClass(CodeBuilder codeModel, ClassBuilder builder, bool withBase, bool methodsOnly, string name1, string name2, bool empty, bool withEmptyClasses)
		{
			...
			if (!empty)
			{
				...
				// test overload
				CreateOverloadMethod(codeModel, builder, false, CodeParameterDirection.In, name1);
				CreateOverloadMethod(codeModel, builder, true, CodeParameterDirection.Ref, name1);
				CreateOverloadMethod(codeModel, builder, false, CodeParameterDirection.Out, name1);
				CreateOverloadMethod(codeModel, builder, false, CodeParameterDirection.In, name2);
				CreateOverloadMethod(codeModel, builder, true, CodeParameterDirection.Ref, name2);
				CreateOverloadMethod(codeModel, builder, false, CodeParameterDirection.Out, name2);
				...
				block.Append(
					codeModel.Assign(
						...
						codeModel.ExtCall(
							...
							new ICodeExpression[]
							{
								codeModel.Array(_typeNotNull, true, false, _value, _value),
								lambda.Method
							})));
			}
		...
		private static void CreateOverloadMethod(CodeBuilder codeModel, ClassBuilder builder, bool isStatic, CodeParameterDirection direction, string name)
		{
			...
			if (direction == CodeParameterDirection.Out)
			{
				method.Body()
		...
		private static void CreateMethod(CodeBuilder codeModel, ClassBuilder builder, bool isStatic, string name1, string name2)
		{
			...
			method.Parameter(codeModel.Parameter(_type, codeModel.Name(name1), CodeParameterDirection.In));
			method.Parameter(codeModel.Parameter(_type, codeModel.Name(name1), CodeParameterDirection.In));
			method.Parameter(codeModel.Parameter(_type, codeModel.Name(name2), CodeParameterDirection.In));
			method.Parameter(codeModel.Parameter(_type, codeModel.Name(name2), CodeParameterDirection.In));
