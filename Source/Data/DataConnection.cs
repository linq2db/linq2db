    ...
    public partial class DataConnection : ICloneable
    {
        ...
        internal IDataReader ExecuteReader(CommandBehavior commandBehavior)
        {
            while (true)
            {
                try
                {
                    return ExecuteReaderRunner(GetCommandBehavior(commandBehavior));
                }
                catch (Exception intercept)
                {
                    var intercepted = ExceptionIntercept(intercept);
        ...
        IDataReader ExecuteReaderRunner(CommandBehavior commandBehavior)
        {
            if (TraceSwitch.Level == TraceLevel.Off)
