...
{
	/// <summary>
	/// This visitor used to walk all generated code and fix identifiers to be valid according to F# naming
	/// and conflict resolution rules. Keyword escaping (double-backtick) is handled by the code generator at
	/// emit time; this visitor only normalizes characters and resolves name conflicts, mirroring the C#
	/// normalizer (F# identifier character rules match the ones used here).
	/// </summary>
	internal sealed class FSharpNameNormalizationVisitor : NoopCodeModelVisitor
	{
		...
		public FSharpNameNormalizationVisitor(ILanguageProvider languageProvider)
		{
		...
		protected override void Visit(CodeClass @class)
		{
			...
			// track current type name in type members
		...
		/// <summary>
		/// Normalize identifier name by removing unsupported characters in unsupported positions.
		...
		private string NormalizeIdentifier(string name)
		{
			...
			// if identifier starts with two (or more) underscores: remove all but one
		...
		private static string GetBaseIdentifierName(string identifierName, NameFixOptions? fixOptions, int? position)
		{
			var baseName = fixOptions switch
			{
				...
				_ =>
					throw new NotImplementedException($"F# name validator doesn't implement {fixOptions.FixType} name fix strategy"),
			};
