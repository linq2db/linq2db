...
namespace LinqToDB.Linq.Builder
{
	...
	class TakeSkipBuilder : MethodCallBuilder
	{
		...
		static void BuildTake(ExpressionBuilder builder, IBuildContext sequence, ISqlExpression expr, TakeHints? hints)
		{
			...
			if (hints != null && !builder.DataContextInfo.DataContext.SqlProviderFlags.GetIsTakeHintsSupported(hints.Value))
			...
			if ( sql.Select.SkipValue != null &&
				 builder.DataContextInfo.DataContext.SqlProviderFlags.IsTakeSupported &&
				!builder.DataContextInfo.DataContext.SqlProviderFlags.GetIsSkipSupportedFlag(sql))
			{
			...
			if (!builder.DataContextInfo.DataContext.SqlProviderFlags.GetAcceptsTakeAsParameterFlag(sql))
			{
		...
		static void BuildSkip(ExpressionBuilder builder, IBuildContext sequence, ISqlExpression prevSkipValue, ISqlExpression expr)
		{
			...
			{
				if (builder.DataContextInfo.DataContext.SqlProviderFlags.GetIsSkipSupportedFlag(sql) ||
					!builder.DataContextInfo.DataContext.SqlProviderFlags.IsTakeSupported)
					sql.Select.Take(builder.Convert(
						sequence,
