				...
				case "+":
				{
					if (element.Expr1.SystemType == typeof(string) && element.Expr2.SystemType != typeof(string))
					{
						...
						return new SqlBinaryExpression(
							...
							element.Expr1,
							element.Operation,
							(ISqlExpression)Visit(PseudoFunctions.MakeCast(element.Expr2, new DbDataType(typeof(string), DataType.VarChar, length: len.Value))),
							element.Precedence);
					}
					...
					if (element.Expr1.SystemType != typeof(string) && element.Expr2.SystemType == typeof(string))
					{
						...
						return new SqlBinaryExpression(
							element.SystemType,
							(ISqlExpression)Visit(PseudoFunctions.MakeCast(element.Expr1, new DbDataType(typeof(string), DataType.VarChar, length: len.Value))),
							element.Operation,
							element.Expr2,
							element.Precedence);
					}
