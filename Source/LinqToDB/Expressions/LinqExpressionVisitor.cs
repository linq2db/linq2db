...
namespace LinqToDB.Expressions
{
	...
	internal class LinqExpressionVisitor
	{
		readonly ISet<Expression> _visitedExpressions = new HashSet<Expression>();
		readonly IDictionary<Expression, Expression> _visitedTransformExpressions = new Dictionary<Expression, Expression>();
		...
		void Visit<T>(IEnumerable<T> source, Action<Expression> func)
			where T : Expression
		{
			foreach (var item in source)
				Visit(item, func);
		...
		/// </summary>
		public void Visit(Expression expr, Action<Expression> func)
		{
			if (expr == null || !_visitedExpressions.Add(expr))
				return;
		...
		void Visit<T>(IEnumerable<T> source, Func<T,bool> func)
		{
			foreach (var item in source)
				func(item);
		}

		void Visit<T>(IEnumerable<T> source, Func<Expression,bool> func)
			where T : Expression
		{
			foreach (var item in source)
				Visit(item, func);
		...
		/// </summary>
		public void Visit(Expression expr, Func<Expression,bool> func)
		{
			if (expr == null || _visitedExpressions.Contains(expr) || !func(expr))
				return;

			_visitedExpressions.Add(expr);
		...
		Expression Find<T>(IEnumerable<T> source, Func<T,Expression> func)
		{
			foreach (var item in source)
			{
				var ex = func(item);
		...
		Expression Find<T>(IEnumerable<T> source, Func<Expression,bool> func)
			where T : Expression
		{
			foreach (var item in source)
			{
		...
		/// </summary>
		public Expression Find(Expression expr, Func<Expression,bool> func)
		{
			if (expr == null || func(expr))
				return expr;

			if (!_visitedExpressions.Add(expr))
				return null;
		...
		IEnumerable<T> Transform<T>(ICollection<T> source, Func<Expression,Expression> func)
			where T : Expression
		{
			var modified = false;
			var list     = new List<T>();
		...
		public Expression Transform(Expression expr, [InstantHandle] Func<Expression,Expression> func)
		{
			if (expr == null)
				return null;

			if (_visitedTransformExpressions.ContainsKey(expr))
				return _visitedTransformExpressions[expr];

			var ex = func(expr);
			if (ex != expr)
			{
				_visitedTransformExpressions.Add(expr, ex);
				return ex;
			}

			_visitedTransformExpressions.Add(expr, expr);
		...
		Expression TransformXE(Expression expr, Func<Expression,Expression> func)
		{
			return expr;
		}

		Expression TransformX(TryExpression e, Func<Expression,Expression> func)
		{
		...
		Expression TransformX(SwitchExpression e, Func<Expression,Expression> func)
		{
		...
		Expression TransformX(DynamicExpression e, Func<Expression,Expression> func)
		{
		...
		Expression TransformX(ChangeTypeExpression e, Func<Expression,Expression> func)
		{
		...
		Expression TransformXInit(NewArrayExpression e, Func<Expression,Expression> func)
		{
		...
		Expression TransformX(NewArrayExpression e, Func<Expression,Expression> func)
		{
			var ex = Transform(e.Expressions, func);
		...
		Expression TransformX(MemberInitExpression e, Func<Expression,Expression> func)
		{
		...
		Expression TransformX(MemberExpression e, Func<Expression,Expression> func)
		{
			var ex = Transform(e.Expression, func);
		...
		Expression TransformX(ListInitExpression e, Func<Expression,Expression> func)
		{
		...
		Expression TransformX(LambdaExpression e, Func<Expression,Expression> func)
		{
			var b = Transform(e.Body,       func);
		...
		Expression TransformX(MethodCallExpression e, Func<Expression,Expression> func)
		{
		...
		Expression TransformX(UnaryExpression e, Func<Expression,Expression> func)
		{
			var o = Transform(e.Operand, func);
			return o != e.Operand ? Expression.MakeUnary(e.NodeType, o, e.Type, e.Method) : e;
		}

		Expression TransformX(BinaryExpression e, Func<Expression,Expression> func)
		{
		...
		IEnumerable<T> Transform2<T>(ICollection<T> source, Func<T,T> func)
			where T : class
		{
			var modified = false;
			var list     = new List<T>();
		...
		IEnumerable<T> Transform2<T>(IEnumerable<T> source, Func<Expression,TransformInfo> func)
			where T : Expression
		{
			var modified = false;
			var list     = new List<T>();
		...
		public Expression Transform(Expression expr, Func<Expression,TransformInfo> func)
		{
			if (expr == null)
				return null;
			...
			{
				_visitedTransformExpressions.Add(expr, ti.Expression);
