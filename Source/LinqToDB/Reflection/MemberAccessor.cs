						...
						Expression MakeGetter(Expression ex, int i)
						{
							...
							if (next.Type.IsClass || next.Type.IsNullable())
							{
								...
								return Expression.Block(
									...
									Expression.IfThen(
										Expression.NotEqual(local, ExpressionHelper.UntypedNull),
				...
				//
				{
							...
							void MakeSetter(Expression ex, int i)
							{
								...
								else
								{
									if (next.Type.IsClass || next.Type.IsNullable())
									{
										...
										exprs.Add(
											Expression.IfThen(
												Expression.Equal(local, ExpressionHelper.UntypedNull),
												Expression.Block(
					...
					else
					{
						...
						SetterExpression = Expression.Lambda(
							Expression.Block(
								new[] { fakeParam },
								Expression.Assign(fakeParam, ExpressionHelper.Constant(0))),
							objParam,
							valueParam);
					}
				}
		...
		void SetSimple(MemberInfo memberInfo, EntityDescriptor? ed)
		{
			...
			else
			{
				...
				SetterExpression = Expression.Lambda(
					...
					Expression.Block(
						new[] { fakeParam },
						new Expression[] { Expression.Assign(fakeParam, ExpressionHelper.Constant(0)) }),
					objParam,
					valueParam);
			}
		}
