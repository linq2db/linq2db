using System;
using System.Threading.Tasks;

namespace LinqToDB.Interceptors
{
	using Common.Internal;

	public abstract class DataContextInterceptor : IDataContextInterceptor
	{
		public virtual void OnClosed      (DataContextEventData eventData) { }
		public virtual void OnClosing     (DataContextEventData eventData) { }
		public virtual Task OnClosedAsync (DataContextEventData eventData) => TaskCache.CompletedTask;
		public virtual Task OnClosingAsync(DataContextEventData eventData) => TaskCache.CompletedTask;
	}
}
