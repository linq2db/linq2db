...
namespace LinqToDB.Linq.Relinq
{
	...
	{
		private class QueryGenerationVisitor : ExtendedQueryModelVisitorBase
		{
			[JetBrains.Annotations.NotNull]
			public IDataContext DataContext { get; }

			public AssociationResolverVisitor AssociationResolverVisitor { get; }


			public MappingSchema MappingSchema => DataContext.MappingSchema;
			public SqlMapper SqlMapper { get; }

			public PropertyMappingRegistry MappingRegistry { get; } = new PropertyMappingRegistry();

			public QueryGenerationVisitor([JetBrains.Annotations.NotNull] IDataContext dataContext,
				AssociationResolverVisitor associationResolverVisitor)
			{
			...
			public QueryGenerationVisitor([JetBrains.Annotations.NotNull] IDataContext dataContext, [JetBrains.Annotations.NotNull] SqlMapper sqlMapper,
				[JetBrains.Annotations.NotNull] AssociationResolverVisitor associationResolverVisitor)
			{
				DataContext = dataContext ?? throw new ArgumentNullException(nameof(dataContext));
				SqlMapper = sqlMapper ?? throw new ArgumentNullException(nameof(sqlMapper));
				AssociationResolverVisitor = associationResolverVisitor ?? throw new ArgumentNullException(nameof(associationResolverVisitor));
			}

			SqlTableSource _currentTableSource;

			public QueryModel CurrentQueryModel { get; private set; }
			public SelectQuery CurrentQuery { get; private set; }
			public IQuerySource CurrentQuerySource { get; private set; }

			private ISqlTableSource GenerateTableSource(IQuerySource querySource, Expression expression, string alias, bool wrapInSource)
			{
				if (alias.StartsWith("<generated"))
					alias = "";
				...
				if (typeof(ITable<>).IsSameOrParentOf(expressionType))
				{
					var tableType = expressionType.GetGenericArgumentsEx()[0];
					source = new SqlTable(MappingSchema, tableType);
				}
				else if (expression is QuerySourceReferenceExpression refernce)
				{
					source = new SqlTable(MappingSchema, refernce.ReferencedQuerySource.ItemType);
				}
				else if (expression is SubQueryExpression subquery)
				{
					var saveQuerySources = CurrentQuerySource;
					CurrentQuerySource = querySource;
					if (typeof(IGrouping<,>).IsSameOrParentOf(subquery.QueryModel.SelectClause.Selector.Type))
					{
					...
					{
						var saveQuery = CurrentQuery;
						VisitQueryModel(subquery.QueryModel);
						source = CurrentQuery;
						CurrentQuery = saveQuery;
					}

					CurrentQuerySource = saveQuerySources;
				}
				else
				{
				...
				if (wrapInSource)
				{
					var tableSource = new SqlTableSource(source, alias);
					SqlMapper.RegisterSource(querySource, expression, tableSource);

					return tableSource;
				}

				SqlMapper.RegisterSource(querySource, expression, source);
				return source;
			}

			public override void VisitMainFromClause(MainFromClause fromClause, QueryModel queryModel)
			{
				var tableSource = (SqlTableSource)GenerateTableSource(fromClause, fromClause.FromExpression, fromClause.ItemName, true);

				CurrentQuery.From.Tables.Add(tableSource);
				_currentTableSource = tableSource;
			...
			{
				BuildSearchCondition(CurrentQuery, CorrectEquality(whereClause.Predicate),
					CurrentQuery.Where.SearchCondition.Conditions);
			...
			public override void VisitResultOperator(ResultOperatorBase resultOperator, QueryModel queryModel, int index)
			{
				switch (resultOperator)
				{
					case GroupResultOperator groupResultOperator:
						{
							var keys = GeneratorHelper.GetGroupByKeys(groupResultOperator.KeySelector, MappingSchema)
								.Select(key => ConvertToSql(CurrentQuery, key))
								.ToArray();
							MappingRegistry.RegisterGroupKey(queryModel, CurrentQuerySource, groupResultOperator.KeySelector, CurrentQuery, null, MappingSchema);
							CurrentQuery.GroupBy.Items.AddRange(keys);
							break;
						}
					case DistinctResultOperator distinctResultOperator:
						{
							CurrentQuery.Select.IsDistinct = true;
							ResolveAllColumns(queryModel);
							break;
						}
					case AnyResultOperator anyResultOperator:
						{
							var newQuery = new SelectQuery();
							var exists = SqlFunction.CreateExists(CurrentQuery);
							newQuery.Select.Add(exists);
					...
					case AllResultOperator allResultOperator:
						{
							var newQuery = new SelectQuery();
							BuildSearchCondition(CurrentQuery, CorrectEquality(Expression.Not(allResultOperator.Predicate)), CurrentQuery.Where.SearchCondition.Conditions);
							var notExists = SqlFunction.CreateNotExists(CurrentQuery);
							newQuery.Select.Add(notExists);
							CurrentQuery = newQuery;
					...
					case CountResultOperator countResultOperator:
						{
							CurrentQuery.Select.Add(SqlFunction.CreateCount(queryModel.SelectClause.Selector.Type, null));
							break;
						}
					case SumResultOperator sumResultOperator:
						{
							var function = new SqlFunction(queryModel.SelectClause.Selector.Type, "SUM", true, ConvertToSql(null, queryModel.SelectClause.Selector));
							CurrentQuery.Select.Add(function);
							break;
						}
					default:
						throw new NotImplementedException();
			...
			private void ResolveAllColumns(QueryModel queryModel)
			{
				GenerateProjection(queryModel.SelectClause.Selector, (e, sql) =>
				{
					CurrentQuery.Select.Add(sql);
					return e;
				}, true);
			}

			public Expression GenerateProjection(Expression selector, Func<Expression, ISqlExpression, Expression> registerExpression, bool enforceServerSide)
			{
				var result = selector.Transform(e =>
				{
					switch (e.NodeType)
					{
						case ExpressionType.New:
							{
								...
								if (newExpr.Members?.Count > 0)
								{
									...
									for (int i = 0; i < newExpr.Members.Count; i++)
									{
										...
										var argument = GenerateProjection(newExpr.Arguments[i], registerExpression, enforceServerSide);
						...
						case ExpressionType.MemberInit:
							{
								...
								var newExpression = (NewExpression)GenerateProjection(memberInit.NewExpression, registerExpression, enforceServerSide);
								var bindings = memberInit.Bindings.Where(b => b.BindingType == MemberBindingType.Assignment)
									.Select(b => Expression.Bind(b.Member, GenerateProjection(((MemberAssignment)b).Expression,
										registerExpression, enforceServerSide))).ToArray();
								return new TransformInfo(memberInit.Update(newExpression, bindings), false);
							}
						case ExpressionType.Extension:
							{
								if (e.CanReduce)
									return new TransformInfo(GenerateProjection(e.Reduce(), registerExpression, enforceServerSide), false);
								if (e is QuerySourceReferenceExpression reference)
								{
									var newExpr = new UnifiedNewExpression(reference, MappingSchema);
									return new TransformInfo(GenerateProjection(newExpr.Reduce(), registerExpression, enforceServerSide), false);
								}

								if (e is SubQueryExpression subQueryExpression)
								{
									var visitor = new QueryGenerationVisitor(DataContext, SqlMapper, AssociationResolverVisitor);
									visitor.VisitQueryModel(subQueryExpression.QueryModel);
									if (registerExpression != null)
									{
										return new TransformInfo(registerExpression(subQueryExpression, visitor.CurrentQuery), false);
									}
								}
								break;
							}
						default:
							{
								if (!enforceServerSide && e is BinaryExpression binary)
								{
									var newBinary = binary.Update(GenerateProjection(binary.Left, registerExpression, false),
										binary.Conversion,
										GenerateProjection(binary.Right, registerExpression, false));
									return new TransformInfo(newBinary, true);
								}

								// that means that we have to convert to reader
								if (enforceServerSide || !CanBeCompiled(e))
								{
			...
			public override void VisitSelectClause(SelectClause selectClause, QueryModel queryModel)
			{
				if (CurrentQuerySource != null)
					MappingRegistry.RegisterSelector(queryModel, CurrentQuerySource, queryModel.SelectClause.Selector, CurrentQuery, _currentTableSource, MappingSchema);
			...
			public override void VisitAdditionalFromClause(AdditionalFromClause fromClause, QueryModel queryModel, int index)
			{
				...
				var tableSource = new SqlTableSource(query, fromClause.ItemName);
				CurrentQuery.From.Tables.Add(tableSource);

				SqlMapper.RegisterSource(fromClause, fromClause.FromExpression, tableSource);

				var saveQuery = CurrentQuery;
				var saveSource = _currentTableSource;
				...
				CurrentQuery = saveQuery;
				_currentTableSource = saveSource;
			}
			...
			public override void VisitJoinClause(ExtendedJoinClause joinClause, QueryModel queryModel, int index)
			{
				...
				{
					case SqlJoinType.Left: joinType = JoinType.Left;
						break;
					case SqlJoinType.Inner: joinType = JoinType.Inner;
						break;
					case SqlJoinType.Right: joinType = JoinType.Right;
						break;
					case SqlJoinType.Full: joinType = JoinType.Full;
						break;
				}

				var innerTableSource = GenerateTableSource(joinClause, joinClause.InnerSequence, joinClause.ItemName, false);

				var joinedTable = new SqlJoinedTable(joinType, innerTableSource, joinClause.ItemName, false);

				BuildSearchCondition(null, CorrectEquality(joinClause.Predicate), joinedTable.Condition.Conditions);

				_currentTableSource.Joins.Add(joinedTable);
			...
			public override void VisitJoinClause(JoinClause joinClause, QueryModel queryModel, int index)
			{
				...
				var joinedTable = new SqlJoinedTable(JoinType.Inner, innerTableSource, joinClause.ItemName, false);

				var predicate =
					ExpressionGeneratorHelper.Equal(MappingSchema, joinClause.OuterKeySelector, joinClause.InnerKeySelector);
				
				BuildSearchCondition(null, CorrectEquality(predicate), joinedTable.Condition.Conditions);
						...
						case ExpressionType.Equal:
							{
								...
								if (binary.Left.Type.IsClassEx())
								{
									if (binary.Left.NodeType == ExpressionType.New)
									{
										...
										var membersRight =
											GeneratorHelper.GetMemberMapping(binary.Right, MappingSchema);

										var newComparison = membersLeft
											.Join(membersRight, l => l.MemberInfo.Name, r => r.MemberInfo.Name,
												(l, r) => Expression.Equal(CorrectEquality(l.Expression),
													CorrectEquality(r.Expression)))
									...
									else
									{
										...
										{
											e = Expression.Equal(CorrectEquality(binary.Left),
												CorrectEquality(binary.Right));
										}
										else
										{
									...
									if (leftSql.CanBeNull)
									{
										...
										if (rightSql.CanBeNull)
										{
											var newExpression = Expression.OrElse(binary, Expression.AndAlso(
												Expression.Equal(CorrectEquality(binary.Left),
													new DefaultValueExpression(MappingSchema, binary.Left.Type)),
												Expression.Equal(CorrectEquality(binary.Right),
													new DefaultValueExpression(MappingSchema, binary.Right.Type))));
					...
					case ExpressionType.Extension:
						{
							if (expression is SubQueryExpression subQueryExpression)
							{
								var visitor = new QueryGenerationVisitor(DataContext, SqlMapper, AssociationResolverVisitor);
								visitor.VisitQueryModel(subQueryExpression.QueryModel);
								if (visitor.CurrentQuery == null)
									throw new InvalidOperationException();

								var expr = new SqlPredicate.Expr.Expr(visitor.CurrentQuery.Select.Columns[0].Expression);

								//TOD: review
								conditions.Add(new SqlCondition(false, expr));
							}
				...
				case ExpressionType.Not    :
					{
						...
						if (notCondition.Conditions.Count == 1)
						{
							...
							else
							{
								...
								conditions.Add(new SqlCondition(isNot, sqlCondition.Predicate));
							}
				...
				return expression.Transform(e =>
				{
					...
					switch (e.NodeType)
					{
						...
						case ExpressionType.MemberAccess:
							{
								var ma = (MemberExpression)e;
								var l  = LinqToDB.Linq.Expressions.ConvertMember(MappingSchema, ma.Expression?.Type, ma.Member);
			...
			bool CanBeCompiled(Expression expr)
			{
				...
				var result = null == expr.Find(ex =>
				{
					...
					switch (ex.NodeType)
					{
						...
						case ExpressionType.Extension:
							{
								return (ex is QuerySourceReferenceExpression) || (ex is SubQueryExpression);
							}
					}
					...
					case ExpressionType.MemberAccess:
						{
							...
							var l  = LinqToDB.Linq.Expressions.ConvertMember(MappingSchema, ex.Expression?.Type, ex.Member);
					...
					case ExpressionType.Call:
						{
							...
							{
								var l = LinqToDB.Linq.Expressions.ConvertMember(MappingSchema, e.Object?.Type, e.Method);
...
//								break;
					...
					case ExpressionType.Extension :
						{
							...
							{
								var visitor = new QueryGenerationVisitor(DataContext, SqlMapper, AssociationResolverVisitor);
								visitor.VisitQueryModel(subquery.QueryModel);
								if (visitor.CurrentQuery == null)
									throw new InvalidOperationException();
								return visitor.CurrentQuery;
							}
			...
			private ISqlExpression ConvertMemberAccessToSql(MemberExpression memberExpression, MemberInfo mi)
			{
				var result = SqlMapper.ResolveExpression(memberExpression, ma =>
				{
					var transformation = MappingRegistry.GetTransformation(memberExpression, MappingSchema);
					if (transformation == null)
					{
//						var corrected = AssociationResolverVisitor.EnsureAssociations(memberExpression, CurrentQueryModel,
//							MappingSchema);
//						if (corrected != memberExpression)
//							return ConvertToSql(null, corrected);

						var tableSource = SqlMapper.GetTableSource(ma.Expression);
						return ConvertTableSourceMemberToSql(tableSource, ma);
					}
					else
					{
						var sql = ConvertToSql(null, transformation.Transformation);
						return transformation.SelectQuery.Select.Columns[transformation.SelectQuery.Select.Add(sql)];
					}
			...
			private ISqlExpression ConvertTableSourceMemberToSql(ISqlTableSource tableSource, MemberExpression memberExpression)
			{
				...
				switch (tableSource)
				{
					...
					case SqlTableSource ts:
						{
							result = ConvertTableSourceMemberToSql(ts.Source, memberExpression);
							break;
						}
					default:
						throw new NotImplementedException();
