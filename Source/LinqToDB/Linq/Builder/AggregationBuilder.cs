			...
			Expression BuildExpression(int fieldIndex, ISqlExpression? sqlExpression)
			{
					...
					if (defaultIfEmpty.DefaultValue != null && expr is ConvertFromDataReaderExpression convert)
					{
						...
						if (expr.Type.IsNullable())
						{
							...
							generator.AddExpression(Expression.IfThen(
								Expression.NotEqual(exprVar, ExpressionHelper.UntypedNull),
				...
				else
				{
					expr = Expression.Block(
						Expression.Call(null, MemberHelper.MethodOf(() => CheckNullValue(false, null!)), Expression.Call(ExpressionBuilder.DataReaderParam, Methods.ADONet.IsDBNull, ExpressionHelper.Constant(0)), Expression.Constant(_methodName)),
