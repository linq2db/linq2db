			...
			else
			{
				...
				if (evaluatedSql is FormattableString formattable)
				{
					...
					arguments = formattable.GetArguments().Select((a, i) =>
					{
						...
						Expression expr = Expression.Call(formatArg, ReflectionHelper.Functions.FormattableString.GetArguments, ExpressionHelper.Constant(i));
...
#endif
				{
					...
					else
					{
						...
						arguments = array.Select((a, i) =>
						{
							...
							Expression expr = Expression.ArrayIndex(arrayExpr, ExpressionHelper.Constant(i));
							if (type != typeof(object))
								expr = Expression.Convert(expr, type);
