...
namespace LinqToDB.Internal.Cache
{
	...
	/// (see <see cref="BoundedCache{TKey,TValue}"/>), which makes "clear all caches" and the entry-count
	...
	/// Dead weak references are pruned during <see cref="ClearAll"/> / <see cref="Snapshot"/> and opportunistically
	/// during <see cref="RegisterScoped{T}"/> so <c>_weak</c> stays bounded even when neither is ever called.
	/// </remarks>
	static class CacheRegistry
	{
		...
		// ClearAll doesn't accumulate wrappers for collected owners (e.g. churned MappingSchemas).
		...
		/// <summary>Returns each live registered cache's name and current entry count, and prunes dead weak references.</summary>
		public static IReadOnlyList<(string Name, long Count)> Snapshot()
		{
			var result = new List<(string, long)>();

			foreach (var cache in _strong.Keys)
				result.Add((cache.Name, cache.Count));
