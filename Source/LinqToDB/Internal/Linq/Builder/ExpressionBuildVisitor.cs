...
#pragma warning restore RS0059 // Do not add multiple public overloads with optional parameters
		...
		public bool HandleAsParameter(Expression node, [NotNullWhen(true)] out Expression? translated)
		{
			...
			{
				var sqlParam = Builder.ParametersContext.BuildParameter(BuildContext, node, CurrentDescriptor, alias : _alias);
		...
		public bool HandleValue(Expression node, [NotNullWhen(true)] out Expression? translated)
		{
			if (CanTryHandleValue(node))
			{
				...
				if (needParameter)
				{
					...
					{
						sql = Builder.ParametersContext.BuildParameter(BuildContext, toTranslate, CurrentDescriptor, alias : _alias);
					}
				}
		...
		bool HandleBinaryMath(BinaryExpression node, out Expression translated)
		{
			...
			using (var saveColumnDescriptor = UsingColumnDescriptor(columnDescriptor))
			{
		...
		ISqlPredicate? MakeIsTrueCheck(ISqlExpression sqlExpression)
		{
			...
			using var saveDescriptor = UsingColumnDescriptor(descriptor);
			...
			if (predicate == null)
			{
				if (isEquality != null)
				{
						...
						&& !(expression.ElementType == QueryElementType.SqlValue && ((SqlValue)expression).Value == null))
					{
						...
						using var saveDescriptorNested = UsingColumnDescriptor(QueryHelper.GetColumnDescriptor(expression));
		...
		ISqlPredicate? ConvertEnumConversion(Expression left, SqlPredicate.Operator op, Expression right)
		{
			...
			switch (value.NodeType)
			{
				...
				case ExpressionType.ConvertChecked:
				{
					...
					using var saveColumnDescriptor = UsingColumnDescriptor(SuggestColumnDescriptor(operand, value));
		...
		private ISqlPredicate? ConvertInPredicate(MethodCallExpression expression)
		{
			...
			using var saveColumnDescriptor = UsingColumnDescriptor(QueryHelper.GetColumnDescriptor(expr));
		...
		ISqlPredicate? CreateStringPredicate(MethodCallExpression expression, SqlPredicate.SearchString.SearchKind kind, ISqlExpression? caseSensitive)
		{
			...
			using var saveColumnDescriptor = UsingColumnDescriptor(descriptor);
