	...
	sealed class UpsertBuilder : MethodCallBuilder
	{
			...
			ExpressionBuilder builder, MethodCallExpression methodCall, BuildInfo buildInfo)
		{
			...
			foreach (var cd in entityDescriptor.Columns)
			{
				...
				// call for dynamic columns) for user .Set/.Ignore field selectors.
				// GetMemberAccessExpression resolves flat/nested via the dot-path walk and dynamic
				// via Sql.Property; GetGetterExpression would instead wrap nested access in a
				// null-check block and emit the raw dynamic getter, neither of which matches the
				// user selector.
				...
				var fieldExpr = cd.GetMemberAccessExpression(contextRef);
				...
				var valueExpr = insertOverride != null
					...
					: cd.GetMemberAccessExpression(itemConst);
				...
				var updFieldExpr = cd.GetMemberAccessExpression(contextRef);
