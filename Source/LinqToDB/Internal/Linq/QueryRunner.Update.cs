	...
	static partial class QueryRunner
	{
		public static class Update<T>
		{
				...
				TableOptions           tableOptions)
			{
				...
				return ei == null ? 0 : (int)ei.GetElement(dataContext, new RuntimeExpressionsContainer(Expression.Constant(obj)), null, null, null)!;
			}
				...
				CancellationToken      token)
			{
				...
				await using (ActivityService.StartAndConfigureAwait(ActivityID.UpdateObjectAsync))
				{
					...
					var result = ei == null ? 0 : await ei.GetElementAsync(dataContext, new RuntimeExpressionsContainer(Expression.Constant(obj)), null, null, null, token).ConfigureAwait(false);
