...
namespace LinqToDB.Internal.SqlQuery
{
	public abstract class SqlPredicate : QueryElement, ISqlPredicate
	{
		...
		{
			public Not(ISqlPredicate predicate) : base(LinqToDB.SqlQuery.Precedence.LogicalNegation)
			{
				Predicate = predicate;
			}
		...
		{
			public TruePredicate() : base(LinqToDB.SqlQuery.Precedence.Primary)
			{
			}
		...
		{
			public FalsePredicate() : base(LinqToDB.SqlQuery.Precedence.Primary)
			{
			}
		...
		public sealed class ExprExpr : Expr
		{
			public ExprExpr(ISqlExpression exp1, Operator op, ISqlExpression exp2, bool? unknownAsValue, bool notNullableExpr1 = false, bool notNullableExpr2 = false)
				: base(exp1, LinqToDB.SqlQuery.Precedence.Comparison)
			{
				Operator         = op;
				Expr2            = exp2;
		...
		public sealed class Like : BaseNotExpr
		{
			public Like(ISqlExpression exp1, bool isNot, ISqlExpression exp2, ISqlExpression? escape, string? functionName = null)
				: base(exp1, isNot, LinqToDB.SqlQuery.Precedence.Comparison)
			{
				Expr2     = exp2;
				Escape    = escape;
		...
		public sealed class SearchString : BaseNotExpr
		{
			...
			public override int GetElementHashCode()
			{
				return HashCode.Combine(
					...
					Expr2.GetElementHashCode(),
					Kind,
					CaseSensitive.GetElementHashCode()
				);
			}
		...
		public sealed class Exists : SqlPredicate
		{
			public Exists(bool isNot, SelectQuery subQuery)
				: base(isNot ? LinqToDB.SqlQuery.Precedence.LogicalNegation : LinqToDB.SqlQuery.Precedence.Primary)
			{
				IsNot    = isNot;
				SubQuery = subQuery;
			}
