			...
			protected int GetType(Type? type)
			{
				...
				if (!ObjectIndices.TryGetValue(type, out var idx))
				{
					...
					else
					{
						...
						Append(LinqToDB.Common.Configuration.LinqService.SerializeAssemblyQualifiedName ? type.AssemblyQualifiedName : type.FullName);
					}
...
string.Create(CultureInfo.InvariantCulture, $"TypeIndex or TypeArrayIndex ({TypeIndex} or {TypeArrayIndex}) expected, but was {typecode}")),
			...
			protected Type? ResolveType(string? str)
			{
				...
				if (type == null)
				{
					...
					if (type == null)
					{
						...
						if (type == null)
						{
							if (LinqToDB.Common.Configuration.LinqService.ThrowUnresolvedTypeException)
		...
		public sealed class QueryDeserializer : DeserializerBase
		{
			...
			DataOptions DeserializeOptions(DataOptions options)
			{
				...
				var disableTimeSpanConversion      = ReadBool();
				...
				options = options
					...
					.WithOptions<SqlOptions>(so =>
						so.WithEnableConstantExpressionInOrderBy(enableConstantExprInOrderBy)
							...
							.WithDisableBuiltInTimeSpanConversion(disableTimeSpanConversion));
