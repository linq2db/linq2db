	...
	public class DuckDBSchemaProvider : SchemaProviderBase
	{
		...
		protected override List<DataTypeInfo> GetDataTypes(DataConnection dataConnection)
		{
			return new List<DataTypeInfo>
			{
				...
				new() { TypeName = "BITSTRING",    DataType = typeof(BitArray).      AssemblyQualifiedName! },
				...
				new() { TypeName = "TIME_NS",      DataType = typeof(TimeSpan).      AssemblyQualifiedName! },
				new() { TypeName = "TIMETZ",       DataType = typeof(DateTimeOffset).AssemblyQualifiedName! },
		...
		protected override Type? GetSystemType(string? dataType, string? columnType, DataTypeInfo? dataTypeInfo, int? length, int? precision, int? scale, GetSchemaOptions options)
		{
			return dataType?.ToUpperInvariant() switch
			{
				...
				"BLOB"         => typeof(byte[]),
				"BITSTRING"    => typeof(BitArray),
				"DATE"         => typeof(DateTime),
				"TIME"         => typeof(TimeSpan),
				"TIME_NS"      => typeof(TimeSpan),
