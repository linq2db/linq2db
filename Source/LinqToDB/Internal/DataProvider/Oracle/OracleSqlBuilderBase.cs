...
namespace LinqToDB.Internal.DataProvider.Oracle
{
	public abstract partial class OracleSqlBuilderBase : BasicSqlBuilder<OracleOptions>
	{
		...
		/// <summary>
		/// Whether the identity column of <paramref name="table"/> is generated by the server itself rather than by
		/// an auxiliary sequence and trigger. A <see cref="SequenceNameAttribute"/> on the identity member declares
		/// the column to be sequence-driven, which is the pre-Oracle-12 scheme, so such tables keep it.
		/// </summary>
		protected virtual bool UseNativeIdentity(SqlTable table) => false;

		/// <summary>
		/// Name of the sequence backing <paramref name="table"/>'s identity column when it is not a native identity.
		/// </summary>
		protected string GetIdentitySequenceName(SqlTable table)
		{
			return GetSequenceNameAttribute(table, false)?.SequenceName ?? MakeIdentitySequenceName(table.TableName.Name);
		}

		string? GetIdentitySequenceSchema(SqlTable table)
		{
			return GetSequenceNameAttribute(table, false)?.Schema ?? table.TableName.Schema;
		}
		...
		public override int CommandCount(SqlStatement statement)
		{
			switch (statement)
			{
				...
				case SqlCreateTableStatement createTable:
					...
					if (_identityField != null && !UseNativeIdentity(createTable.Table!))
						return 3;
					break;
			}
		...
		protected override void BuildDropTableStatement(SqlDropTableStatement dropTable)
		{
			...
			if (identityField != null && UseNativeIdentity(dropTable.Table))
				identityField = null;
			...
			else
			{
				...
				void BuildInnerDropSequence()
				{
					...
					AppendSchemaPrefix(StringBuilder, GetIdentitySequenceSchema(table));
					Convert(StringBuilder, GetIdentitySequenceName(table), ConvertType.SequenceName);
				}
		...
		protected override void BuildCommand(SqlStatement statement, int commandNumber)
		{
			switch (Statement)
			{
				case SqlTruncateTableStatement truncate:
					var sequenceName = GetIdentitySequenceName(truncate.Table!);
		...
		protected override bool TryConvertParameterToSql(SqlParameterValue paramValue)
		{
			...
			if (paramValue.ProviderValue is byte[] { Length: > 2000 }
				|| (paramValue.ProviderValue is string str && (str.Length > 4000 || Encoding.UTF8.GetByteCount(str) > 4000)))
			{
				return false;
			}
