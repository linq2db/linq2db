...
namespace LinqToDB.Internal.DataProvider.Oracle
{
	...
	public class OracleSchemaProvider : SchemaProviderBase
	{
		...
		protected override List<ColumnInfo> GetColumns(DataConnection dataConnection, GetSchemaOptions options)
		{
			...
			var isIdentitySql = _majorVersion switch
			{
				>= 12 => "	CASE c.IDENTITY_COLUMN WHEN 'YES' THEN 1 ELSE 0 END as IsIdentity,",
				_     => "	0                                                   as IsIdentity,",
			};

			var sql = IncludedSchemas.Count != 0 || ExcludedSchemas.Count != 0
				// This is very slow
				? $"""
					SELECT
		...
		{
			return (columnType, precision) switch
			{
				("NUMBER", 0) => columnType,
				_ => base.GetDbType(options, columnType, dataType, length, precision, scale, udtCatalog, udtSchema, udtName),
			};
		}
