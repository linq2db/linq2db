...
namespace LinqToDB.Internal.DataProvider.Oracle
{
	public class Oracle12SqlBuilder : OracleSqlBuilderBase
	{
		...
		protected override bool UseNativeIdentity(SqlTable table) => GetSequenceNameAttribute(table, false) == null;

		protected override void BuildCreateTableIdentityAttribute1(SqlField field)
		{
			if (field.Table is SqlTable table && !UseNativeIdentity(table))
				return;
		...
		protected override void BuildCommand(SqlStatement statement, int commandNumber)
		{
			...
			if (statement is SqlTruncateTableStatement truncate && commandNumber == 1 && UseNativeIdentity(truncate.Table!))
			{
				...
				Convert(StringBuilder, table.IdentityFields[0].PhysicalName, ConvertType.NameToQueryField);
				StringBuilder.AppendLine(" GENERATED BY DEFAULT AS IDENTITY (START WITH 1))");
				return;
			}
