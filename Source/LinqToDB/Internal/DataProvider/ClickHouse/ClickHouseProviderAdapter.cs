...
#pragma warning restore CA1508 // Avoid dead conditional code
		...
		internal static class ClientWrappers
		{
			...
			public sealed class ClickHouseConnectionStringBuilder : TypeWrapper
			{
				...
				{
					// [0]: get UseSession
					(ClickHouseConnectionStringBuilder this_) => this_.UseSession,
					// [1]: set UseSession
					PropertySetter((ClickHouseConnectionStringBuilder this_) => this_.UseSession),
					// [2]: ToString
					(ClickHouseConnectionStringBuilder this_) => this_.ToString()!,
				};
			...
			internal sealed class ClickHouseBulkCopy : TypeWrapper, IDisposable
			{
				...
				{
					// [0]: Dispose
					new Tuple<LambdaExpression, bool>((ClickHouseBulkCopy this_) => ((IDisposable)this_).Dispose(), true),
					// [1]: get BatchSize
					new Tuple<LambdaExpression, bool>((ClickHouseBulkCopy this_) => this_.BatchSize, true),
					// [2]: set BatchSize
					new Tuple<LambdaExpression, bool>(PropertySetter((ClickHouseBulkCopy this_) => this_.BatchSize), true),
					// [3]: get MaxDegreeOfParallelism
					new Tuple<LambdaExpression, bool>((ClickHouseBulkCopy this_) => this_.MaxDegreeOfParallelism, true),
					...
					new Tuple<LambdaExpression, bool>((ClickHouseBulkCopy this_) => this_.DestinationTableName, true),
					...
					// [7]: get RowsWritten
					new Tuple<LambdaExpression, bool>((ClickHouseBulkCopy this_) => this_.RowsWritten, true),
					// [8]: WriteToServerAsync
					new Tuple<LambdaExpression, bool>((ClickHouseBulkCopy this_, IDataReader dataReader, CancellationToken cancellationToken) => this_.WriteToServerAsync(dataReader, cancellationToken), true),
					// [9]: InitAsync
					new Tuple<LambdaExpression, bool>((ClickHouseBulkCopy this_) => this_.InitAsync(), false),
					// [10]: ColumnNames { set; }
...
#pragma warning restore RS0030 //  API mapping must preserve type
		...
		public static class OctonicaWrappers
		{
			...
			public class ClickHouseColumnWriter : TypeWrapper, IDisposable, IAsyncDisposable
			{
				...
				{
					// [0]: Dispose
					new Tuple<LambdaExpression, bool>((ClickHouseColumnWriter this_) => this_.Dispose(), true),
					// [1]: EndWrite
					new Tuple<LambdaExpression, bool>((ClickHouseColumnWriter this_) => this_.EndWrite(), true),
					// [2]: EndWriteAsync
					new Tuple<LambdaExpression, bool>((ClickHouseColumnWriter this_, CancellationToken cancellationToken) => this_.EndWriteAsync(cancellationToken), true),
					// [3]: WriteTable
					new Tuple<LambdaExpression, bool>((ClickHouseColumnWriter this_, IReadOnlyList<object> columns, int rowCount) => this_.WriteTable(columns, rowCount), true),
					// [4]: WriteTableAsync
					new Tuple<LambdaExpression, bool>((ClickHouseColumnWriter this_, IReadOnlyList<object> columns, int rowCount, CancellationToken cancellationToken) => this_.WriteTableAsync(columns, rowCount, cancellationToken), true),

					// [5]: DisposeAsync
					new Tuple<LambdaExpression, bool>
					((ClickHouseColumnWriter this_) => this_.DisposeAsync(), true),
					// [6]: ConfigureColumn
					new Tuple<LambdaExpression, bool>((ClickHouseColumnWriter this_, int ordinal, ClickHouseColumnSettings columnSettings) => this_.ConfigureColumn(ordinal, columnSettings), true),
				};
			...
			internal sealed class ClickHouseException : TypeWrapper
			{
				...
				[
						// [0]: get ErrorCode
						((ClickHouseException this_) => this_.ErrorCode),
				];
