...
#pragma warning restore CA1508 // Avoid dead conditional code
		...
		internal class NpgsqlConnection : TypeWrapper
		{
			private static LambdaExpression[] Wrappers { get; } =
			{
				...
				(NpgsqlConnection this_) => this_.PostgreSqlVersion,
			};
		...
		public class NpgsqlBinaryImporter : TypeWrapper
		{
				...
				= new object[]
			{
				...
				// [0]: Cancel
				new Tuple<LambdaExpression, bool>((NpgsqlBinaryImporter this_) => this_.Cancel(), true),
				// [1]: Complete: pre-v5
				new Tuple<LambdaExpression, bool>((NpgsqlBinaryImporter this_) => this_.Complete(), true),
				// [2]: Complete: v5+
				new Tuple<LambdaExpression, bool>((NpgsqlBinaryImporter this_) => this_.Complete5(), true),
				// [3]: Dispose
				                                  (NpgsqlBinaryImporter this_) => this_.Dispose(),
				// [4]: StartRow
				                                  (NpgsqlBinaryImporter this_) => this_.StartRow(),
				// [5]: CompleteAsync
				new Tuple<LambdaExpression, bool>
				((NpgsqlBinaryImporter this_, CancellationToken token) => this_.CompleteAsync(token),         true),
				// [6]: DisposeAsync
				new Tuple<LambdaExpression, bool>
				((NpgsqlBinaryImporter this_)                          => this_.DisposeAsync(),               true),
				// [7]: StartRowAsync
				new Tuple<LambdaExpression, bool>
				((NpgsqlBinaryImporter this_, CancellationToken token) => this_.StartRowAsync(token), true),
				// [8]: WriteAsync
				new Tuple<LambdaExpression, bool>
				((NpgsqlBinaryImporter this_, object? value, NpgsqlDbType type, CancellationToken token) => this_.WriteAsync(value, type, token), true),
				// [9]: Write
				(NpgsqlBinaryImporter this_, object? value, NpgsqlDbType type) => this_.Write(value, type),
				// [10]: Write
				(NpgsqlBinaryImporter this_, object? value, string dataTypeName) => this_.Write(value, dataTypeName),
				// [11]: WriteAsync
				new Tuple<LambdaExpression, bool>
				((NpgsqlBinaryImporter this_, object? value, string dataTypeName, CancellationToken token) => this_.WriteAsync(value, dataTypeName, token), true),
			};
