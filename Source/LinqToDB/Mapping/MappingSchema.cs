...
#pragma warning restore CA2214 // Do not call overridable methods in constructors
		...
		sealed class DefaultMappingSchema : LockedMappingSchema
		{
			...
			{
				AddScalarType(typeof(char),            new DbDataType(typeof(char), DataType.NChar, 1));
			...
			if (underlyingType.IsEnum)
			{
				...
				else
				{
					...
					if (dt.DataType == DataType.NVarChar && minLen == length)
						return new DbDataType(valueType, DataType.NChar, length.Value);

					if (length.HasValue && dt.DataType.IsCharDataType())
						return new DbDataType(valueType, dt.DataType, length.Value);
