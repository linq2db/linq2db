...
namespace LinqToDB.DataProvider.SqlServer
{
	...
	public abstract class SqlType
	{
		...
		sealed class SqlExpressionAttribute(string name) : Sql.ExpressionAttribute(ProviderName.SqlServer, name);
		...
		[SqlExpression("decimal({0})")]        public static SqlType<decimal?>        Decimal(int precision)            => new SqlType<decimal?>(string.Create(CultureInfo.InvariantCulture, $"decimal({precision})"));
		[SqlExpression("decimal({0}, {1})")]   public static SqlType<decimal?>        Decimal(int precision, int scale) => new SqlType<decimal?>(string.Create(CultureInfo.InvariantCulture, $"decimal({precision}, {scale})"));
		...
		[SqlExpression("float({0})")]          public static SqlType<double?>         Float(int n)                      => new SqlType<double?>  (string.Create(CultureInfo.InvariantCulture, $"float({n})"));
		...
		[SqlExpression("datetimeoffset({0})")] public static SqlType<DateTimeOffset?> DatetimeOffset(int size)          => new SqlType<DateTimeOffset?>(string.Create(CultureInfo.InvariantCulture, $"datetimeoffset({size})"));
		[SqlExpression("datetime2")]           public static SqlType<DateTime?>       Datetime2()                       => new SqlType<DateTime?>      ("datetime2");
		[SqlExpression("datetime2({0})")]      public static SqlType<DateTime?>       Datetime2(int size)               => new SqlType<DateTime?>      (string.Create(CultureInfo.InvariantCulture, $"datetime2({size})"));
		...
		[SqlExpression("time({0})")]           public static SqlType<TimeSpan?>       Time(int size)                    => new SqlType<TimeSpan?>      (string.Create(CultureInfo.InvariantCulture, $"time({size})"));

		// Character strings
		//
		[SqlExpression("char")]                public static SqlType<string?>         Char()                            => new SqlType<string?>("char");
		[SqlExpression("char({0})")]           public static SqlType<string?>         Char(int size)                    => new SqlType<string?>(string.Create(CultureInfo.InvariantCulture, $"char({size})"));
		...
		[SqlExpression("varchar({0})")]        public static SqlType<string?>         VarChar(int size)                 => new SqlType<string?>(string.Create(CultureInfo.InvariantCulture, $"varchar({size})"));
		...
		[SqlExpression("nchar({0})")]          public static SqlType<string?>         NChar(int size)                   => new SqlType<string?>(string.Create(CultureInfo.InvariantCulture, $"nchar({size})"));
		...
		[SqlExpression("nvarchar({0})")]       public static SqlType<string?>         NVarChar(int size)                => new SqlType<string?>(string.Create(CultureInfo.InvariantCulture, $"nvarchar({size})"));
		...
		[SqlExpression("binary({0})")]         public static SqlType<byte[]?>         Binary(int size)                  => new SqlType<byte[]?>(string.Create(CultureInfo.InvariantCulture, $"binary({size})"));
		...
		[SqlExpression("varbinary({0})")]      public static SqlType<byte[]?>         VarBinary(int size)               => new SqlType<byte[]?>(string.Create(CultureInfo.InvariantCulture, $"varbinary({size})"));
		...
		// Vectors
		[SqlExpression("vector({0},float32)")] public static SqlType<float[]?>        Vector32(int size)                => new (string.Create(CultureInfo.InvariantCulture, $"vector({size},float32)"));
#if NET8_0_OR_GREATER
		[SqlExpression("vector({0},float16)")] public static SqlType<Half[]?>         Vector16(int size)                => new (string.Create(CultureInfo.InvariantCulture, $"vector({size},float16)"));
#endif
	}
