			...
			if (!isNull)
			{
				switch (Type.GetTypeCode(cast.SystemType.ToUnderlying()))
				{
					...
					case TypeCode.DateTime :
						...
						{
							return new SqlCastExpression(new SqlExpression(cast.Expression.SystemType, "Extend({0}, Hour to Second)", Precedence.Primary, argument), new DbDataType(typeof(string), DataType.Char, length: 8), null, true);
						}
