...
- `configs` folder stores one config per test job, staged as `UserDataProviders.json` for whichever target framework a leg runs. A job's provider list does not depend on the framework, so each file holds a single `Azure.TestJob` section and `DataProviders.json` points every `<TFM>.Azure` configuration at it - adding a .NET version costs nothing here. `hana2.json` is a placeholder: the SAP HANA config is generated by `scripts/hana2.sh`, which only learns the ODBC driver path at run time
...
- :arrow_right: - no Windows job for this target framework: the leg also has a Linux job, which already runs the modern TFMs for it, so `templates/test-jobs.yml` sets `modern_tfms: false` on the Windows matrix entry. `netfx` is unaffected - it runs nowhere but Windows
