...
# Start all three MySQL-family servers, each on its own host port (matching CommonConnectionStrings:
# mysql latest -> 3306, mysql 5.7 -> 3307, mariadb -> 3316), so all five MySql/MariaDB providers run
# as concurrent lanes within a single CI job.
...
docker run -d --name mariadb -e MYSQL_ROOT_PASSWORD=root -p 3316:3306 mariadb:12
docker ps -a
