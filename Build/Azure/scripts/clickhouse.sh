...
# network_compression_method - 26.9 changed the default to ZSTD(3) for client/server traffic, and
# Octonica.ClickHouseClient implements LZ4 only: every read fails with "An unexpected compression
# algorithm identifier was received. Expected value: 0x82. Actual value: 0x90.". Drop once it reads ZSTD.
docker exec clickhouse sed -i "0,/<\/default>/{s/<\/default>/<join_use_nulls>1<\/join_use_nulls><mutations_sync>1<\/mutations_sync><allow_experimental_object_type>1<\/allow_experimental_object_type><allow_experimental_geo_types>1<\/allow_experimental_geo_types><allow_experimental_json_type>1<\/allow_experimental_json_type><allow_experimental_correlated_subqueries>1<\/allow_experimental_correlated_subqueries><enable_analyzer>1<\/enable_analyzer><enable_materialized_cte>1<\/enable_materialized_cte><network_compression_method>LZ4<\/network_compression_method><\/default>/}" /etc/clickhouse-server/users.xml
